...
fn datatype_identity_naming_and_serde_are_total() {
    ...
    // `Isin`, and the five string layouts were appended after it, which is
    // what `as_u8` being a wire contract requires.
    assert_eq!(DataTypeId::ALL.last(), Some(&DataTypeId::LargeStringView));
