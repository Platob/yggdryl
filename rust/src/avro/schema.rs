...
//! typed: a `date` int decodes as a calendar [`Date32`](crate::Date32)
...
impl Parser {
    ...
    /// `metadata` and `value`, both `bytes`, in either order.
    fn is_variant_record(fields: &[FieldType]) -> bool {
        matches!(
            fields,
            [first, second]
                if ((first.name == crate::VARIANT_METADATA_FIELD
                        && second.name == crate::VARIANT_VALUE_FIELD)
                    || (first.name == crate::VARIANT_VALUE_FIELD
                        && second.name == crate::VARIANT_METADATA_FIELD))
                    && matches!(first.schema, Node::Bytes)
                    && matches!(second.schema, Node::Bytes)
        )
    }
