...
const DEFAULT_BATCH_ROWS: usize = crate::media::DEFAULT_RECORD_BATCH_ROW_SIZE;
...
impl ColumnReader {
    ...
    fn new(node: &Node, schema: &Schema, arrow: &ArrowDataType) -> crate::Result<Self> {
        Ok(match node {
            ...
            Node::Record(record) if record.variant => {
        ...
        match self {
            ...
            Self::Uuid { builder, fixed } => {
                let stored = if *fixed {
                    ...
                    crate::uuid_parse(cursor.string()?.as_bytes())?
                };
                builder
                    .append_value(stored)
                    .map_err(|error| invalid(format_smolstr!("{error}")))?;
...
fn encode_cell(
    ...
    match node {
        ...
        Node::Uuid => {
            ...
            let spelling = crate::uuid_text(stored);
        ...
        Node::Record(record) if record.variant => {
            ...
            for field in &record.fields {
                let child = entries.column_by_name(&field.name).ok_or_else(|| {
                    invalid(format_smolstr!(
                        "expected a variant child named {:?}",
                        field.name
                    ))
                })?;
                put_bytes(payload, child.as_binary::<i32>().value(row));
            }
        }
        Node::Record(record) => {
            let entries = column.as_struct();
