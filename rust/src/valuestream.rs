...
/// The one version of the stream, the first byte of every one.
    ...
    fn value(&mut self, depth: usize) -> Result<Scalar> {
        ...
        Ok(match id {
            ...
            DataTypeId::Variant => {
                ...
                Scalar::Variant(crate::Variant::new(metadata, value)?)
            }
            DataTypeId::Struct => {
                let count = self.size()?;
...
impl Scalar {
    ...
    /// This value as the value stream, whole.
    #[must_use]
    pub fn into_value_bytes(&self) -> Vec<u8> {
        let mut bytes = Vec::with_capacity(16);
        for chunk in self.encode_value_stream_bytes() {
