...
#[doc(hidden)]
pub mod internals {
    //! What `rust/tests/arrow/rows.rs` pins and a caller cannot reach.
    //!
    //! `reader` is the one widening every record I/O adapter goes through, and
    //! what it pulls, when it pulls it, and what it answers after a row that
    //! does not convert is the whole contract; a caller reaches it only as the
    //! reader an I/O method already handed back.
    ...
    /// # Errors
    ///
    /// Returns an error when `field` is not a record root.
    pub fn reader<I, R>(
        field: &Field,
        rows: I,
        batch_row_size: Option<usize>,
