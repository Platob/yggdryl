        ...
        for (index, field) in fields.iter().enumerate() {
            ...
            let time = match field.as_digest().time() {
                Some(path) => {
                    ...
                    if !crate::hashing::txhash::arrow::accepts_time(selection.field.dtype()) {
                        return Err(digest_metadata_error(
                            DIGEST_TIME_KEY,
                            &field_path,
                            format!(
...
fn default_holder_algorithm(field: &Field) -> Option<DigestAlgorithm> {
    if field.as_digest().time().is_some() {
        return crate::hashing::txhash::coupled_holder_algorithm(field);
    }
    match field.dtype() {
...
fn fill_struct<S: ArrowDigestState>(
    ...
    for holder in &plan.holders {
        ...
        let unix = match &holder.time {
            Some(time) => Some(crate::hashing::txhash::arrow::unix_selection(
                &columns,
                plan.fields,
        ...
        let computed = match (&holder.time, &unix) {
            (Some(time), Some(unix)) => {
                ...
                crate::hashing::txhash::arrow::collect(
                    unix,
                    &values,
                    None,
                    time.unit,
