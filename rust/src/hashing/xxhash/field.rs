...
use crate::hashing::txhash::{TIME, UNIT};
...
pub(crate) fn holder_accepts(field: &Field, algorithm: DigestAlgorithm) -> bool {
    if field.as_digest().time().is_some() {
        return crate::hashing::txhash::coupled_holder_accepts(field, algorithm);
    }
    match algorithm {
...
pub(crate) fn expected_holder_dtypes(field: &Field, algorithm: DigestAlgorithm) -> String {
    if field.as_digest().time().is_some() {
        return crate::hashing::txhash::expected_coupled_dtype(algorithm);
    }
    match algorithm {
...
impl DigestField<'_> {
    ...
    pub fn apply_arrow_batch(
        ...
        Ok(crate::hashing::xxhash::arrow::apply_arrow_batch_with(
