...
//! [`crate::hashing::xxhash::arrow`] answers for the same rows, and the two are laid
//! side by side per row. Nothing here hashes on its own - a row's digest half
//! is exactly [`row_digests`](crate::hashing::xxhash::arrow::row_digests) of that
...
//! column's nulls.
//!
//! At one fixed unit and algorithm, byte ordering is chronological within
//! each sign range. The raw two's-complement count places negative instants
//! after all nonnegative instants; it is not a sign-normalized sort key.
...
use crate::hashing::xxhash::arrow::{
...
///
/// The digest half is [`row_digests`](crate::hashing::xxhash::arrow::row_digests) of
...
/// use yggdryl::{DigestAlgorithm, TimeUnit, hashing::txhash};
///
/// # fn main() -> Result<(), Box<dyn std::error::Error>> {
...
/// // These nonnegative instants share a unit and algorithm, so their bytes
/// // sort in time order and keep identical rows at different instants apart.
...
/// assert_eq!(&digests, &yggdryl::hashing::xxhash::arrow::row_digests(&batch, DigestAlgorithm::Xxh3)?);
/// # Ok(())
/// # }
/// ```
///
...
///
/// The digest half is
/// [`column_digests`](crate::hashing::xxhash::arrow::column_digests) of the array
...
/// use yggdryl::hashing::xxhash::arrow::column_digests;
/// use yggdryl::{DataType, DigestAlgorithm, Field, TimeUnit, hashing::txhash};
///
/// # fn main() -> Result<(), Box<dyn std::error::Error>> {
...
/// the width [`row_digests`](crate::hashing::xxhash::arrow::row_digests) answers;
