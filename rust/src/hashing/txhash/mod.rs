...
//!
//! A [`TxHash`] is a big-endian unix count followed by a [`Digest`](crate::Digest)
//! at its algorithm's exact width. With a fixed unit and algorithm, its bytes
//! sort by time and then content within either sign range; negative counts
//! sort after all nonnegative counts. The instant is always UTC and counted
//! in microseconds unless a resolution is named. The digest is whatever
//! [`crate::hashing::xxhash`] answers for the same bytes - this module defines
//! no second hash, only the coupling.
//!
//! Value ordering compares the unit first, then the signed count and digest;
//! it does not normalize instants across resolutions. The raw bytes carry
//! neither unit nor algorithm; decoding supplies both.
//!
//! ```
//! use yggdryl::{DigestAlgorithm, hashing::txhash, hashing::xxhash};
//!
...
//!
//! The digest half is xxHash, and everything [`crate::hashing::xxhash`] says of it
...
///
/// ```
/// use yggdryl::{DigestAlgorithm, hashing::txhash};
///
