...
use crate::{DateTimeType, DecimalType, TimeType};
...
struct PublishedHintFailure {
    inner: crate::fs::MemoryFileSystem,
...
impl crate::fs::FileSystem for PublishedHintFailure {
...
impl crate::fs::FileSystem for SameVersionWinner {
    fn type_name(&self) -> &str {
        self.inner.type_name()
    }
...
struct SameVersionWriter {
    inner: Box<dyn ByteWriter>,
    filesystem: crate::fs::MemoryFileSystem,
    path: String,
    bytes: Vec<u8>,
    armed: Arc<AtomicBool>,
    injections: Arc<AtomicUsize>,
...
fn listed_paths(folder: &crate::fs::Folder) -> Vec<String> {
...
impl crate::fs::FileSystem for RefusedDocumentWrite {
    fn type_name(&self) -> &str {
        self.inner.type_name()
    }
...
impl crate::fs::FileSystem for MetadataOnlyWinner {
    fn type_name(&self) -> &str {
        self.inner.type_name()
    }
...
mod types {
    ...
    fn a_string_in_another_charset_is_refused_by_name() {
        ...
        assert!(!crate::iceberg::value::is_portable(&latin));
    }
    ...
    fn a_code_bound_is_portable_and_round_trips_as_a_string_datum() {
        ...
        for (dtype, value) in [
            ...
            let bytes = crate::iceberg::value::single_value(&exact, &dtype)
...
mod partition_specs {
    ...
    fn a_partition_directory_is_spelled_the_way_every_other_lake_spells_it() {
        ...
        assert_eq!(
            crate::media::partition::partition_text(&Scalar::date32(19_723)).unwrap(),
            "2024-01-01"
        );
    }
    ...
    fn an_evolved_schema_keeps_the_old_one_and_numbers_above_it() {
        ...
        evolved
            .set_dtype(crate::DataType::from(
                crate::StructType::from_fields(fields).unwrap(),
            ))
            .unwrap();
...
mod planning {
    ...
    use crate::DataType;
    use crate::iceberg::schema::assign_field_ids;
    use crate::local::Folder;
...
fn a_reported_hint_failure_reconciles_to_the_version_fresh_handles_see() {
    ...
    let folder = crate::fs::Folder::from_path(filesystem.clone(), "bucket/table", None).unwrap();
    let mut table = Table::create(
        folder.clone(),
        FormatVersion::V2,
        trade_schema(),
...
mod line_projection {
    ...
    fn regex_typed_text_rows_stream_into_a_table_through_record_media() {
        ...
        let mut schema = source
            ...
            .unwrap()
            .into_scheme_compat(&crate::Scheme::ICEBERG)
            .unwrap();
        assert_eq!(
        ...
        assert_eq!(
            ...
            &crate::DataType::decimal128(20, 0).unwrap()
        );
...
mod isolation {
    ...
    fn a_time_partition_range_skips_manifests_like_the_equality_form() {
        let filesystem: Arc<dyn crate::fs::FileSystem> =
            Arc::new(crate::fs::MemoryFileSystem::new());
        filesystem
            .create_dir("isolation-timepartition", true)
            .unwrap();
        let folder =
            crate::fs::Folder::from_path(filesystem, "isolation-timepartition", None).unwrap();
        let mut schema = StructType::from_fields([
            DataType::Int64.required_field("id"),
            DataType::DateTime(DateTimeType::DateTime64 {
                unit: TimeUnit::Microsecond,
        ...
        table.set_options(
            IcebergOptions::new()
                .try_with_write_staging(crate::iceberg::WriteStaging::Off)
                .unwrap(),
        );
        let arrow = schema.into_arrow_schema().unwrap();
    ...
    fn unknown_is_null_in_both_directions_and_only_a_v3_table_carries_it() {
        ...
        let stored =
            crate::parquet::read_field(&handle, &crate::parquet::ParquetOptions::new()).unwrap();
        ...
        reopened.metadata().validate().unwrap();
        let text =
            crate::json::into_utf8(&reopened.metadata().clone().into_json().unwrap()).unwrap();
    ...
    fn variant_is_the_semi_structured_datatype_in_both_directions_and_rides_a_data_file() {
        ...
        assert_eq!(
            reader
                ...
                .map(String::as_str),
            Some(crate::VARIANT_EXTENSION_NAME)
        );
        let reopened = Table::open(Folder::new(&v3).unwrap()).unwrap();
        assert_eq!(
            reopened.schema().unwrap().fields()[1].dtype(),
...
/// pinned in `object::tests::accounting`.
