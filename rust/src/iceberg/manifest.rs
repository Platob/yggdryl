...
fn official_reader_view(
    ...
    crate::avro::write_container(&mut output, schema, &metadata_refs, rows)?;
...
fn scalar_from_official(value: &OfficialLiteral, dtype: &OfficialType) -> Result<Scalar> {
    ...
    match (dtype, value) {
        ...
        (OfficialPrimitiveType::Uuid, OfficialPrimitiveLiteral::UInt128(value)) => {
            DataType::uuid().scalar(crate::uuid_text(&value.to_be_bytes()))
        }
...
#[doc(hidden)]
pub mod internals {
    //! What `rust/tests/iceberg/manifest.rs` pins and a caller cannot reach.
    //!
    //! A caller writes manifests and reads them back, so the only way to pin
    //! what the readers refuse is to write a manifest they should refuse -
    //! which means building the Avro record schema and the one row by hand,
    //! exactly as the writer would, and then bending it.

    use crate::iceberg::{FormatVersion, ManifestEntry};
    use crate::{Field, Result, Scalar};

    /// The Avro record schema one manifest's entries are written against.
    pub fn manifest_entry_schema(version: FormatVersion, partition: &Field) -> Result<Scalar> {
        super::manifest_entry_schema(version, partition)
    }

    /// One manifest entry as the Avro row a writer would emit for it.
    pub fn entry_to_value(
        entry: &ManifestEntry,
        version: FormatVersion,
        partition: &Field,
    ) -> Result<Scalar> {
        super::entry_to_value(entry, version, partition)
    }

    /// The Avro map schema one metric column is written under.
    pub fn avro_map(key_id: i32, value_id: i32, value_type: &str) -> Result<Scalar> {
        super::avro_map(key_id, value_id, value_type)
    }

    /// Whether an Avro schema document declares a `fixed[16]` UUID anywhere.
    pub fn contains_fixed_uuid(value: &Scalar) -> bool {
        super::contains_fixed_uuid(value)
    }

    /// The rewritten container the official reader is handed, when the schema
    /// holds a `fixed[16]` UUID the official reader cannot name.
    pub fn fixed_uuid_official_reader_view(bytes: &[u8]) -> Result<Option<Vec<u8>>> {
        super::fixed_uuid_official_reader_view(bytes)
    }
}
