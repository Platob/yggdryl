...
//!
//! Only the *document vocabulary* differs per API, so that is what each owner
//! reads for itself: `s3::aws::xml` reads `ListBucketResult`,
//! `s3::azure::xml` reads `EnumerationResults`, `aws::sts` reads
//! `AssumeRoleResponse`. This module reads none of them, and knows the name
//! of no element.

use std::fmt;

/// Nesting past which a scan stops. The deepest document any of these APIs
/// answers is four levels, so this bounds recursion on a hostile body without
/// touching a real one.
...
pub(crate) fn parse_document(xml: &[u8]) -> Result<Element, XmlError> {
    ...
    let unmarked = text.strip_prefix('\u{FEFF}').unwrap_or(text);
    let mut scanner = Scanner {
        text: unmarked,
        at: 0,
        base: text.len() - unmarked.len(),
    };
    scanner.skip_misc()?;
...
struct Scanner<'a> {
    /// The whole document, past any byte order mark.
    text: &'a str,
    /// The byte offset of the next unread character.
    at: usize,
    /// The bytes of the mark that was stripped, so an offset in a refusal
    /// counts from the start of the input.
    base: usize,
}
...
impl<'a> Scanner<'a> {
    ...
    fn error(&self, message: impl fmt::Display) -> XmlError {
        XmlError(format!("{message} at byte {}", self.base + self.at))
    }
...
pub mod internals {
    ...
    //! refuses is the whole contract. The module is private, so [`Element`]
    //! and [`XmlError`] being `pub` reaches nobody; the reading itself
    //! forwards, and changes no visibility.
    pub use super::{Element, XmlError};

    /// The root element of `xml`, prolog and trailing comments allowed.
    ///
    /// # Errors
    ///
    /// Malformed XML, or anything but comments outside the root element.
    pub fn parse_document(xml: &[u8]) -> Result<Element, XmlError> {
        super::parse_document(xml)
    }
    ...
    /// # Errors
    ///
    /// As [`parse_document`], or a root of another name.
    pub fn parse_root(xml: &[u8], expected: &str) -> Result<Element, XmlError> {
        super::parse_root(xml, expected)
    }
}
