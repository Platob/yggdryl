...
#[doc(hidden)]
pub mod internals {
    //! What `rust/tests/object/xml.rs` and the two dialect readers' suites pin
    //! and a caller cannot reach.
    //!
    //! The scanner is the reading every XML answer goes through, and what it
    //! refuses is the whole contract. `object::xml` is a private module of a
    //! published one, so [`Element`] and [`XmlError`] being `pub` reaches
    //! nobody; the readings themselves forward, and change no visibility.
    pub use super::{Element, XmlError};

    use crate::object::answer::ErrorBody;
