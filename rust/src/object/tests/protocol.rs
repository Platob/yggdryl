...
use crate::object::{AwsOptions, Credentials, File, ObjectOptions};
...
fn every_request_carries_a_signature_over_the_headers_it_names() {
    ...
    assert_eq!(payload_hash, crate::object::sigv4::sha256_hex(b"PAR1"));
}
...
fn a_location_naming_no_container_is_refused_before_anything_is_built() {
    let error = crate::object::file("file:///tmp/part.parquet").expect_err("a refusal");
    ...
    crate::object::folder("https://example.com/x").expect_err("a refusal");
    // A store's own word for it is what its own refusal uses: a location that
    // names only an endpoint has named no container yet.
    let error = crate::object::file("gs://storage.googleapis.com/").expect_err("a refusal");
    assert!(error.to_string().contains("naming a bucket"), "{error}");
    let error = crate::object::file("az://trades.blob.core.windows.net/").expect_err("a refusal");
    assert!(error.to_string().contains("naming a container"), "{error}");
}
...
fn every_s3_url_spelling_reaches_the_same_object() {
    ...
    for scheme in ["s3", "s3a", "s3n"] {
        ...
        let mut handle = crate::object::file_with(&url, options(&store)).expect("an object handle");
        ...
        let listed = crate::object::folder_with(&prefix, options(&store))
            .expect("a prefix handle")
            .ls(false, false)
        ...
        assert_eq!(
            crate::object::located_with(&url, options(&store))
                .expect("a location")
                .kind(),
            IOKind::File
        );
    ...
    for scheme in ["s3", "s3a", "s3n"] {
        let url = format!("{scheme}://{BUCKET}/lake/{scheme}.bin");
        let refused = crate::object::file_with(&url, options(&store))
...
fn a_write_signs_its_payload_over_http_and_leaves_it_unsigned_over_tls() {
    ...
    assert_eq!(
        ...
        Some(crate::object::sigv4::sha256_hex(b"AAPL,187.23").as_str()),
    );
...
fn an_empty_value_is_one_put_however_low_the_multipart_threshold() {
    ...
    let client =
        std::sync::Arc::new(crate::object::client::Client::new(&url, options).expect("a client"));
...
fn a_named_role_is_traded_for_a_session_once_and_signs_everything_after() {
    ...
    let role = crate::object::AssumedRole::new("arn:aws:iam::123456789012:role/lake-reader")
...
fn a_bucket_lifecycle_this_client_may_not_perform_costs_no_request() {
    ...
    let client = std::sync::Arc::new(
        crate::object::client::Client::new(&url, refusing()).expect("a client"),
    );
    let mut ledger = crate::object::Folder::new(client, url).expect("a prefix handle");
