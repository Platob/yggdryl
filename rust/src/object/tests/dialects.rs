...
fn a_large_value_is_uploaded_in_chunks_and_assembles_to_what_was_written() {
    ...
    for provider in EVERY {
        ...
        let client = std::sync::Arc::new(
            crate::object::client::Client::new(
                &url,
        ...
        let mut handle = crate::object::File::new(client, url).expect("an object handle");
...
fn a_container_is_created_and_removed_through_every_store() {
    ...
    for provider in EVERY {
        ...
        let url = crate::Url::from_str(&format!("{}://{name}/", provider.scheme().as_str()))
            .expect("a location");
        let client = std::sync::Arc::new(
            crate::object::client::Client::new(&url, options_for(&store, provider))
                .expect("a client"),
        );
        let mut folder = crate::object::Folder::new(client, url).expect("a container handle");
...
fn a_location_resolves_to_what_it_names_on_every_store() {
    ...
    for provider in EVERY {
        ...
        let leaf = crate::object::Path::new(client, url).expect("a location");
        ...
        let prefix = crate::object::Path::new(client, url).expect("a location");
