...
mod representation {
    ...
    fn a_decimal_holds_a_fraction_no_double_can_hold() {
        ...
        assert_eq!(Value::d128(i128::MAX, 0).as_d128(), Some((i128::MAX, 0)));
    }

    #[test]
    fn one_renderer_restores_the_exact_plain_text() {
        assert_eq!(
            Value::d128(1_050, 2).as_decimal_utf8().as_deref(),
            Some("10.50")
        );
        assert_eq!(
            Value::d128(-5, 3).as_decimal_utf8().as_deref(),
            Some("-0.005")
        );
        assert_eq!(
            Value::d128(12, -2).as_decimal_utf8().as_deref(),
            Some("1200")
        );
        assert_eq!(
            Value::d256(crate::I256::from_i128(1_050), 2)
                .as_decimal_utf8()
                .as_deref(),
            Some("10.50")
        );
...
mod comparison {
    ...
    fn two_spellings_of_one_number_are_one_value() {
        for (left, right) in [
            (Value::d128(1_050, 2), Value::d128(105, 1)),
            (Value::d128(0, 7), Value::d128(0, -7)),
            (Value::d128(-1_050, 2), Value::d128(-105, 1)),
            (Value::d128(100, 0), Value::d128(1, -2)),
        ] {
    ...
    fn decimals_order_by_the_number_they_name() {
        assert!(Value::d128(1, 1) < Value::d128(2, 1));
        assert!(Value::d128(-1, 0) < Value::d128(1, 5));
        // 0.001 is smaller than 1 even though its coefficient is not.
        assert!(Value::d128(1, 3) < Value::d128(1, 0));
        // A coefficient too wide to restate is by that fact the larger one.
        assert!(Value::d128(i128::MAX, 0) > Value::d128(1, -30));
        assert!(Value::d128(i128::MIN, 0) < Value::d128(-1, -30));
    }
    ...
    fn a_decimal_is_its_own_kind_and_never_an_integer() {
        ...
        assert_ne!(Value::d128(1, 0), Value::from(1_i64));
        assert!(Value::from(1_i64) < Value::d128(1, 0));
    }
}
...
mod restating {
    ...
    fn restating_adds_digits_freely_and_drops_none() {
        ...
        assert_eq!(Value::d128(i128::MAX, 0).decimal_unscaled_at(1), None);
