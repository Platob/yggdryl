...
mod tests {
    ...
    mod the_registry {
        ...
        fn zones_are_sorted_so_the_binary_search_is_valid() {
            let names: Vec<&str> = crate::timezone::ZONES
                .iter()
                .map(|zone| zone.name)
                .collect();
        ...
        fn aliases_are_sorted_and_never_shadow_a_real_zone() {
            let names: Vec<&str> = crate::timezone::ALIASES
                .iter()
                .map(|(from, _)| *from)
                .collect();
            let mut sorted = names.clone();
            ...
            for (from, _) in crate::timezone::ALIASES {
                assert!(
                    crate::timezone::zone(from).is_none(),
        ...
        fn every_offset_is_a_whole_number_of_minutes_within_a_day() {
            for zone in crate::timezone::ZONES {
                assert_eq!(
                    zone.standard % 60,
                    0,
    ...
    mod canonicalization {
        ...
        fn every_alias_interns_to_its_canonical_handle() {
            for &(alias, canonical) in crate::timezone::ALIASES {
