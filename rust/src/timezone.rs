...
mod value {
    ...
    impl Timezone {
        ...
        pub fn from_offset(seconds: i32) -> Result<Self> {
            ...
            Ok(Self(crate::timezone::fixed_handle(seconds)))
        }

        /// Return the canonical name without allocating.
        pub fn as_str(&self) -> &str {
            crate::timezone::name(self.0)
        }
        ...
        pub fn as_smol_str(&self) -> &SmolStr {
            crate::timezone::smol_str(self.0)
        }
    ...
    impl FromStr for Timezone {
        ...
        fn from_str(value: &str) -> Result<Self> {
            ...
            if let Some(handle) = crate::timezone::registered(value) {
                return Ok(Self(handle));
            }
            if let Some(canonical) = crate::timezone::alias(value) {
                return Ok(Self(crate::timezone::intern(canonical)?));
            }
            if let Some(handle) = crate::timezone::registered_ignoring_case(value) {
                return Ok(Self(handle));
            }
            ...
            Ok(Self(crate::timezone::intern(value)?))
        }
    }
...
pub mod internals {
    ...
    pub struct ZoneRow {
        ...
        /// Seconds east of UTC when no saving is in force.
        pub standard: i32,
        /// The seconds the zone saves, when it observes a rule at all.
        pub saving: Option<i32>,
        /// The abbreviation used inside a saving period.
        pub daylight_abbreviation: Option<&'static str>,
    }

    /// Every zone this build knows the current rules for, in table order.
    #[must_use]
    pub fn zones() -> Vec<ZoneRow> {
        super::ZONES
            .iter()
            .map(|zone| ZoneRow {
                name: zone.name,
                standard: zone.standard,
                saving: zone.saving.map(|saving| saving.save),
                daylight_abbreviation: zone.daylight_abbreviation,
            })
            .collect()
    }

    /// Every alias this build resolves, in table order.
    #[must_use]
    pub fn aliases() -> &'static [(&'static str, &'static str)] {
        super::ALIASES
    }

    /// Whether a name is a canonical zone of the bundled registry.
    #[must_use]
    pub fn is_zone(name: &str) -> bool {
        super::zone(name).is_some()
    }
