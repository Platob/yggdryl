...
use super::{
    Attribute, Bound, Bounds, ColumnBounds, Expression, Filter, Literal, Projection, Residual,
    Safety, Selector, Term,
};
use crate::{
...
fn a_selector_computes_rows_on_both_tiers() {
    ...
    assert_eq!(
        ...
        crate::arrow::arrow_schema_from_field(&output)
            .unwrap()
            .as_ref()
    );
    ...
    for (position, row) in rows.iter().enumerate() {
        ...
        let held: Vec<Scalar> = output
            ...
            .map(|(field, column)| {
                crate::arrow::value::value_from_array(field.dtype(), column.as_ref(), position)
                    .unwrap()
            })
            .collect();
...
fn a_partition_declaration_is_a_transform() {
    ...
    year.as_partition_mut()
        .set_transform(super::Function::Year)
        .unwrap();
...
fn every_attribute_declares_a_cost_and_a_type() {
    for attribute in Attribute::ALL {
        ...
        assert_eq!(
            answered.is_null(),
            matches!(attribute.cost(), super::Cost::Stat),
