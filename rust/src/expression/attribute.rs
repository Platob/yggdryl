...
//!
//! Every attribute declares a [`Cost`], and the cost is what makes ordering
...
//! harming correctness - an attribute ordered later is still answered - so an
...
impl Attribute {
    /// Every attribute that takes no argument, in canonical spelling.
    ...
    /// The canonical name of this attribute, without the `&holder.` prefix.
    #[must_use]
    ...
    /// Resolve an argument-free attribute name, ASCII case-insensitively.
    #[must_use]
    pub fn from_name(name: &str) -> Option<Self> {
        let lowered = name.to_ascii_lowercase();
        Self::ALL
            .into_iter()
            .find(|attribute| attribute.as_str() == lowered)
    }

    /// Every attribute name this grammar accepts, for an error message.
    #[must_use]
    ...
    /// What answering this attribute costs.
    #[must_use]
    ...
    /// The datatype this attribute always answers in.
    ///
    /// Fixed per attribute and never inferred, so `&holder.size > 100` compares
    ...
    /// The output field of this attribute, named as it is written.
    ///
    /// Always nullable: a handle with no identifier answers null for every free
    /// attribute rather than failing, because "this holder has no URL" is not a
    /// broken predicate.
    #[must_use]
    ...
    /// Answer this attribute from an identifier alone.
    ///
    /// Every [`Cost::Free`] attribute is answered here. A [`Cost::Stat`] one
...
pub trait Attributes {
    ...
    /// # Errors
    ///
    /// Returns the backing store's failure when a [`Cost::Stat`] attribute
    /// cannot be answered.
...
/// let folder = LocalFolder::temporary()?;
/// let name = Handle(&folder).attribute(&Attribute::Name)?;
/// # let _ = name;
/// # Ok(())
/// # }
/// ```
...
impl<H: IOBase + ?Sized> Attributes for Handle<'_, H> {
    fn attribute(&self, attribute: &Attribute) -> Result<Scalar> {
        read_handle(self.0, attribute)
    }
}

/// Answer one attribute against a live handle.
///
/// Free attributes reach only [`IOBase::url`]; the stat tier is the only place
/// that touches the backing store, and it is reached only when the attribute
/// says so.
///
/// # Errors
///
...
pub fn read_handle<H: IOBase + ?Sized>(handle: &H, attribute: &Attribute) -> Result<Scalar> {
    match attribute.cost() {
        Cost::Free => Ok(handle
            .url()
            .map_or(Scalar::Null, |url| attribute.read_url(url))),
        Cost::Stat => Ok(match attribute {
            Attribute::Size => i64::try_from(handle.size()).map_or(Scalar::Null, Scalar::from),
            Attribute::Kind => Scalar::from(handle.kind().to_string()),
            Attribute::IsContainer => Scalar::from(handle.is_container()),
            Attribute::IsEmpty => Scalar::from(handle.is_empty()),
            // Unreachable by the cost table above, and stated rather than
            // panicked so an attribute added later degrades to "not known".
            _ => Scalar::Null,
        }),
    }
}

/// The error an attribute name that is not in the vocabulary produces.
