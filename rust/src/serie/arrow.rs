...
use super::text::{ByteSerie, ByteViewSerie, FixedSerie, RawRun, TextRun};
...
fn column_of(field: Arc<Field>, array: &ArrayRef, parent: Option<&NullBuffer>) -> Result<Serie> {
    ...
    match array.data_type() {
        ...
        ArrowDataType::Utf8 => Ok(ByteSerie::<Utf8Type, TextRun>::new(
            field,
            held::<GenericByteArray<Utf8Type>>(&field_ref, array)?,
        )
        .into_serie()),
        ArrowDataType::LargeUtf8 => Ok(ByteSerie::<LargeUtf8Type, TextRun>::new(
            field,
            held::<GenericByteArray<LargeUtf8Type>>(&field_ref, array)?,
        )
        .into_serie()),
        ArrowDataType::Utf8View => Ok(ByteViewSerie::<StringViewType, TextRun>::new(
            field,
        ...
        ArrowDataType::Binary => {
            ...
            Ok(if is_text(&dtype) {
                ByteSerie::<BinaryType, TextRun>::new(field, runs).into_serie()
            } else {
                ByteSerie::<BinaryType, RawRun>::new(field, runs).into_serie()
            })
        }
        ArrowDataType::LargeBinary => {
            let runs = held::<GenericByteArray<LargeBinaryType>>(&field_ref, array)?;
            Ok(if is_text(&dtype) {
                ByteSerie::<LargeBinaryType, TextRun>::new(field, runs).into_serie()
            } else {
                ByteSerie::<LargeBinaryType, RawRun>::new(field, runs).into_serie()
            })
        }
        ArrowDataType::BinaryView => {
            let runs = held::<GenericByteViewArray<BinaryViewType>>(&field_ref, array)?;
            Ok(if is_text(&dtype) {
                ByteViewSerie::<BinaryViewType, TextRun>::new(field, runs).into_serie()
            } else {
                ByteViewSerie::<BinaryViewType, RawRun>::new(field, runs).into_serie()
            })
        }
        ArrowDataType::FixedSizeBinary(_) => {
            let runs = held::<FixedSizeBinaryArray>(&field_ref, array)?;
            Ok(if is_text(&dtype) {
                FixedSerie::<TextRun>::new(field, runs).into_serie()
            } else {
                FixedSerie::<RawRun>::new(field, runs).into_serie()
            })
        }
