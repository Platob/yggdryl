...
fn a_code_set_round_trips_canonically_and_a_hand_edit_names_its_byte_position() {
    ...
    assert_eq!(field.get_metadata("FIX:codes"), Some("sidecodeset"));
    let stored = registry
        .codeset_of(&field)
...
fn a_code_set_carries_every_fact_the_specification_states_about_a_member() {
    ...
    assert_eq!(cleared.codesets().count(), 0);
}
...
fn a_replacement_document_round_trips_canonically_and_in_order() {
    ...
    assert_eq!(FixReplacements::render(&owned).unwrap(), RULE80A_DOCUMENT);
}
...
fn the_replacement_writer_refuses_what_the_document_cannot_state() {
    ...
    for plan in [Plan::new(), "select *".parse::<Plan>().unwrap()] {
        let error = FixReplacements::render(&[FixReplacement::new(plan)]).unwrap_err();
        assert!(
    ...
    assert!(
        over.is_err() || FixReplacements::render(&[FixReplacement::new(over.unwrap())]).is_err(),
...
fn a_hand_edited_replacement_document_is_refused_at_its_own_byte() {
    ...
    assert!(FixReplacements::render(&[FixReplacement::from(entry)]).is_err());
}
...
fn every_committed_code_set_is_the_document_the_rust_writer_renders() {
    ...
    // One vocabulary per set rather than one per field reading by it: 2,026
    // fields draw on these 735 sets, 165 of them on one unit set alone, and
    // the 27,209 records that count used to be is what naming them once took
    // out of the store.
    assert_eq!(sets, 735, "code sets held");
    assert_eq!(codes, 7_729, "code records");
}
...
fn a_composed_key_fills_the_field_its_last_segment_names() {
    let codec = crate::FixCodec::new(committed());
    let one = |row: &str| {
        codec
