...
impl<'registry> Builder<'registry> {
    ...
    pub(super) fn push_pairs(&mut self, pairs: &[(&[u8], &[u8])], absent: impl Fn(&[u8]) -> bool) {
        ...
        while let Some((key, value)) = pairs.get(cursor) {
            ...
            if let Some((tag, plan)) = group {
                ...
                let slot = self.slot_for(plan.field().clone(), tag);
    ...
    fn push_flat(&mut self, key: &str, text: &str, raw: &[u8]) {
        ...
        if let Some((group, counter)) = self.counter(key) {
            let slot = self.slot_for(group, counter);
            slot.group = true;
        }
    }
    ...
    fn push_grouped(
        ...
        // Indexed wire counters and logical group names resolve one group
        // slot. The scalar counter retains its own separate field and value.
