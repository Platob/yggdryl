...
//! [`codesets/`](super::FixRegistry::write_into), and a field's `FIX:codes`
...
/// the name a field's `FIX:codes` states. Every read walks the stored
/// document as it goes and hands back slices of it, so nothing is parsed
...
impl FixRegistry {
    ...
    ///
    /// Returns [`Error::InvalidRecord`] when the name is not one a store can
    /// file, and [`Error::Conflict`] when the name is already held.
...
mod codeset_tests {
    ...
    fn a_field_keeps_the_set_it_reads_by_when_another_dictionary_names_another() {
        ...
        assert_eq!(held.codesets().len(), 2);
        assert!(held.get_codeset("venuecodeset").is_some());
    }

    #[test]
    fn a_dictionary_holding_no_set_writes_no_folder_and_reads_back_equal() {
        // Every category folder always exists - the crate's own fields, its
        // two groups and the fixed row see to that - but `codesets/` does not,
        // so a store that never held one is the shape a reader meets first.
        ...
        assert_eq!(registry.codesets().len(), 0);
