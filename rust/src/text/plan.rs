...
//! plan, the eighteen event columns it opens with included, and a projection
...
pub(crate) enum TextSource {
    /// One of the eighteen columns the line is stated in as an event.
    Event(EventColumn),
...
impl TextPlan {
    ...
    pub(crate) fn compile(options: &TextOptions) -> Result<Self> {
        ...
        // The event the line is, in the eighteen columns every graph event
    ...
    pub(crate) fn field(&self, name: SmolStr) -> Result<Field> {
        let fields = self
            ...
            .map(|column| {
                ...
                // holds: the eighteen event columns carry the display their
