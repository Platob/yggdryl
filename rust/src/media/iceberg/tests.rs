...
fn time_travel_reads_a_previous_snapshot_by_id_and_by_ref() {
    ...
    table
        .commit_metadata_changes(|metadata| {
            metadata.set_snapshot_ref("audit", crate::media::iceberg::SnapshotRef::tag(past))
        })
        .unwrap();
...
mod concurrency_and_compaction {
    ...
    fn a_beaten_merge_reports_the_conflict_rather_than_rebasing() {
        ...
        let mut stale = Table::open(Folder::new(&path).unwrap()).unwrap();
        stale.set_options(crate::media::iceberg::IcebergOptions::default().with_commit_retries(1));
    ...
    fn the_cadence_compacts_after_every_n_data_commits_by_itself() {
        ...
        .unwrap();
        table.set_options(
            crate::media::iceberg::IcebergOptions::default().with_compact_after_commits(2),
        );
...
mod manifest_planning {
    ...
    fn an_avro_container_without_required_iceberg_metadata_is_not_a_manifest() {
        ...
        .unwrap();
        let mut handle = Buffer::new();
        crate::media::avro::write_container(&mut handle, &schema, &[], &[row]).unwrap();
...
mod data_mime_type {
    use super::*;
    use crate::MimeType;
    use crate::media::iceberg::IcebergOptions;
...
mod interop_regressions {
    use super::*;
    use crate::media::iceberg::{PartitionField, SchemaUpdate};
        ...
        let spec = PartitionSpec {
            ...
            fields: vec![
                PartitionField {
                    ...
                    field_id: 1000,
                    name: "symbol".into(),
                    transform: crate::media::iceberg::Transform::Identity,
                },
                PartitionField {
                    source_id: 1,
                    field_id: 1001,
                    name: "id_bucket".into(),
                    transform: crate::media::iceberg::Transform::Bucket(4),
                },
            ],
        };
