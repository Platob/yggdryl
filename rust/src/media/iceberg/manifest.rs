...
//! statistics. Writes use [`crate::media::avro`] through the table's handle, never a
...
pub fn write_manifest<H: IOBase + ?Sized>(
    ...
    crate::media::avro::write_container(handle, &avro_schema, &metadata, &rows)
}
...
fn preflight_manifest_entries(bytes: &[u8]) -> Result<()> {
    use crate::media::avro::container;
    use crate::media::avro::datum::{Cursor, DatumCodec};
...
fn manifest_list_version(bytes: &[u8]) -> Result<OfficialFormatVersion> {
    use crate::media::avro::container;
    use crate::media::avro::datum::Cursor;
...
mod official_read_tests {
    ...
    fn write_nonconforming_manifest(
        ...
        crate::media::avro::write_container(handle, avro_schema, &metadata, &[row]).unwrap();
    }
    ...
    fn manifest_spec_rejects_missing_and_malformed_required_metadata() {
        ...
        crate::media::avro::write_container(
            &mut missing_schema,
            &avro_schema,
