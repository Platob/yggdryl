...
pub(crate) fn read_arrow_reader(
    ...
    read_owned_arrow_reader_at(crate::media::source::owned_leaf(handle)?, url, options)
}
...
fn read_owned_arrow_reader_at<H: IOBase + 'static>(
    ...
    let source: Box<dyn Read + Send + 'static> = match handle.bound_location().cloned() {
        ...
        None => Box::new(crate::media::source::SendDecodedReader::new(
...
pub(crate) fn row_size(handle: &(impl IOBase + ?Sized), options: &TextOptions) -> Result<u64> {
    ...
    let source = crate::media::source::DecodedReader::new(raw, codings);
...
impl BoundReader {
    ...
    fn initialize(&mut self) -> std::io::Result<bool> {
        ...
        let mut stream = crate::media::source::fetched(BoundStream::new(stream));
...
pub(crate) fn append_arrow_reader(
    ...
    let mut encoded = Vec::new();
    {
        ...
        if !handle.is_empty() {
            ...
            let mut decoder = codec.reader(crate::media::source::fetched(source));
