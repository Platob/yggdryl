...
/// )])?;
/// let schema = DataType::from(StructType::from_fields([DataType::Int32.required_field("year")])?)
///     .required_field("row");
///
...
fn leaf_options(options: &RecordOptions, pairs: &[(String, String)]) -> Result<RecordOptions> {
    ...
    leaf.set_merge_by(options.merge_by().without_columns(&columns));
    Ok(leaf)
}
...
impl FolderWriter {
    ...
    fn validate_merge_key(options: &RecordOptions, columns: &[String]) -> Result<()> {
        let keys = options.merge_by();
        if keys.is_empty()
            || keys.projections().iter().any(|key| {
                key.term().as_column().is_none_or(|name| {
                    !columns
                        .iter()
        ...
        Err(Error::InvalidRecord {
            ...
            reason: crate::text::expected_got(
                ...
                format_args!("only partition columns `{keys}`, which are constant within a leaf"),
            ),
        })
    }
