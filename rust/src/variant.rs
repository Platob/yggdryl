...
//! which is less. The mapping is total on the way in - every value the
//! crate holds writes, because a leaf the standard has no physical type
//! for takes [the spelling the JSON codec gives it](crate::json), which is
//! the same text or number another reader would see - and exact on the way
//! out for the twenty-one physical types the standard names:
//!
...
//!
//! A decimal past thirty-eight digits, a time or a timestamp whose count
//! is not a whole microsecond, a zoned time, a zoned duration and a
...
impl Variant {
    ...
    /// spelling for: a decimal past thirty-eight digits, a time or a
    /// timestamp that is not a whole microsecond, a zoned time or
    ...
    pub fn encode(value: &Scalar) -> Result<Self> {
        ...
        let index: BTreeMap<&str, u32> = dictionary
            .iter()
            .enumerate()
            .map(|(at, key)| (key.as_str(), at as u32))
            .collect();
        let mut payload = Vec::with_capacity(16);
        write_value(value, &index, 0, &mut payload)?;
        Ok(Self {
            metadata: metadata_bytes(&dictionary)?.into(),
            value: payload.into(),
        })
    }
...
impl<'a> Dictionary<'a> {
    ...
    fn over(metadata: &'a [u8]) -> Result<Self> {
        ...
        let mut offsets = Vec::with_capacity(count.saturating_add(1).min(1 << 16));
        let mut at = 1 + width;
        for index in 0..=count {
            let offset = read_le(metadata, at, width).ok_or_else(|| {
                refuse(at, format_smolstr!("offset {index} of {count} is not held"))
            })?;
            if offsets.last().is_some_and(|last| offset < *last) {
                return Err(refuse(at, "an offset before the one before it"));
            }
            offsets.push(offset);
            at += width;
        }
        let keys = metadata
            .get(at..)
            .ok_or_else(|| refuse(at, "key bytes the metadata ends before"))?;
        if offsets.last().is_some_and(|end| *end > keys.len()) {
            return Err(refuse(
                at,
                format_smolstr!(
                    "{} key bytes announced, {} held",
    ...
    fn key(&self, id: usize, at: usize) -> Result<&'a str> {
        let (Some(start), Some(end)) = (self.offsets.get(id), self.offsets.get(id + 1)) else {
            return Err(refuse(
                at,
                format_smolstr!(
        ...
        std::str::from_utf8(&self.keys[*start..*end])
            .map_err(|_| refuse(at, format_smolstr!("key {id} is not UTF-8")))
    }
}
...
fn write_decimal(out: &mut Vec<u8>, coefficient: i128, scale: i8, floor: u8) -> Result<()> {
    ...
    if coefficient.unsigned_abs() >= 10_u128.pow(DECIMAL_DIGITS) {
        return Err(refuse(
            out.len(),
            format_smolstr!(
    ...
    if floor <= DECIMAL4
        && i128::from(i32::MIN) <= coefficient
        && coefficient <= i128::from(i32::MAX)
    {
        out.push(PRIMITIVE | (DECIMAL4 << 2));
        out.push(scale);
        out.extend_from_slice(&(coefficient as i32).to_le_bytes());
    } else if floor <= DECIMAL8
        && i128::from(i64::MIN) <= coefficient
        && coefficient <= i128::from(i64::MAX)
    {
...
impl<'a> Reader<'a> {
    ...
    fn primitive(&mut self, id: u8, depth: usize) -> Result<Scalar> {
        Ok(match id {
            ...
            BINARY => Scalar::from(self.sized()?.to_vec()),
            STRING => {
    ...
    fn array_value(&mut self, header: u8, depth: usize) -> Result<Scalar> {
        ...
        let offsets = self.offsets(count, width)?;
        let start = self.at;
    ...
    fn object(&mut self, header: u8, depth: usize) -> Result<Scalar> {
        ...
        for _ in 0..count {
            ids.push(self.unsigned(id_width)?);
        }
        let offsets = self.offsets(count, offset_width)?;
        let start = self.at;
        let mut entries = BTreeMap::new();
        for (id, offset) in ids.into_iter().zip(offsets.iter().copied()) {
            let key = SmolStr::new(self.dictionary.key(id, self.at)?);
            self.at = start + offset;
            let value = self.value(depth + 1)?;
            if entries.insert(key.clone(), value).is_some() {
                return Err(refuse(
                    self.at,
                    format_smolstr!("an object naming {key} twice"),
                ));
            }
        }
        self.at = start + offsets[count];
    ...
    /// Read `count + 1` offsets, each of `width` bytes.
    ///
    ...
    fn offsets(&mut self, count: usize, width: usize) -> Result<Vec<usize>> {
        let mut offsets = Vec::with_capacity(count.saturating_add(1).min(1 << 16));
        for _ in 0..=count {
            offsets.push(self.unsigned(width)?);
        }
        ...
        if offsets.iter().any(|offset| *offset > end) {
            return Err(refuse(self.at, "an offset past the end of the values"));
        }
        Ok(offsets)
    }
}
...
mod arrow {
    ...
    pub(crate) fn is_variant_storage(dtype: &ArrowDataType) -> bool {
        ...
        binary(VARIANT_METADATA_FIELD) && binary(VARIANT_VALUE_FIELD)
    }
}
