//! One byte stream for any value: the encoding's version, the value's
...
//! else.
//!
//! [`Scalar::encode_variant_stream_bytes`] answers the stream one chunk
//! at a time, [`Scalar::into_variant_bytes`] the whole of it, and
//! [`Scalar::decode_variant_bytes`] and
//! [`Scalar::decode_variant_stream_bytes`] read either back. A
...
pub const VARIANT_VERSION: u8 = 0;
...
/// The stream [`Scalar::encode_variant_stream_bytes`] answers: one chunk
...
fn encode<'value>(value: &'value Scalar, chunk: &mut Vec<u8>, children: &mut Vec<Child<'value>>) {
    match value {
        ...
        other => write_text(
            chunk,
            other.id(),
            other.as_str().expect("a code borrows its text"),
        ),
    }
}
...
impl Scalar {
    /// This value as the variant encoding, one chunk at a time: the
    ...
    /// The value one variant encoding holds, whole: the version, the
    ...
    pub fn decode_variant_bytes(bytes: &[u8]) -> Result<Self> {
        let mut reader = Reader { bytes, at: 0 };
        let version = reader.byte()?;
        if version != VARIANT_VERSION {
            return Err(reader.refuse(format_smolstr!(
                "version {version} is not the {VARIANT_VERSION} this reads"
            )));
        }
    ...
    /// The value a variant encoding split into chunks holds - what
    /// [`Self::encode_variant_stream_bytes`] answered, or any other cut of
    /// the same bytes - gathered and read as
    /// [`Self::decode_variant_bytes`] reads it.
    ///
    /// # Errors
    ///
    /// [`Self::decode_variant_bytes`] carries the rule.
    pub fn decode_variant_stream_bytes<I>(chunks: I) -> Result<Self>
    where
        I: IntoIterator,
        I::Item: AsRef<[u8]>,
    {
        ...
        Self::decode_variant_bytes(&bytes)
    }
}
...
impl DataType {
    ...
    /// holds.
    pub fn encode_variant_stream_bytes(&self, value: &Scalar) -> Result<VariantStream> {
        Ok(self.cast_scalar(value)?.encode_variant_stream_bytes())
    }
    ...
    /// Returns the cast's refusal where the value is not one this datatype
    /// holds.
    pub fn encode_variant_bytes(&self, value: &Scalar) -> Result<Vec<u8>> {
        Ok(self.cast_scalar(value)?.into_variant_bytes())
    }
    ...
    pub fn decode_variant_bytes(&self, bytes: &[u8]) -> Result<Scalar> {
        self.cast_scalar(&Scalar::decode_variant_bytes(bytes)?)
    }
    ...
    ///
    /// [`Self::decode_variant_bytes`] carries the rule.
    pub fn decode_variant_stream_bytes<I>(&self, chunks: I) -> Result<Scalar>
    where
        I: IntoIterator,
        I::Item: AsRef<[u8]>,
    {
        self.cast_scalar(&Scalar::decode_variant_stream_bytes(chunks)?)
    }
}
...
mod tests {
    ...
    fn a_tree_streams_one_leaf_per_chunk_and_reads_back_whole() {
        ...
        assert_eq!(chunks.concat(), value.into_variant_bytes());
        assert_eq!(Scalar::decode_variant_stream_bytes(&chunks).unwrap(), value);
    }

    #[test]
    fn the_refusals_name_the_byte() {
        let error = Scalar::decode_variant_bytes(&[1, 0])
            .unwrap_err()
            .to_string();
        assert!(error.contains("version 1"), "{error}");
        let error = Scalar::decode_variant_bytes(&[0, 0x10])
            .unwrap_err()
            .to_string();
        assert!(
        ...
        let error = Scalar::decode_variant_bytes(&[0, DataTypeId::Int32.as_u8(), 1])
            .unwrap_err()
            .to_string();
        assert!(error.contains("4 bytes announced"), "{error}");
        let error = Scalar::decode_variant_bytes(&[0, 0, 0])
            .unwrap_err()
            .to_string();
