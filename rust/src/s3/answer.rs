...
#[doc(hidden)]
pub mod internals {
    //! What `rust/tests/s3/xml.rs`, `rust/tests/s3/aws/xml.rs` and
    //! `rust/tests/s3/azure/xml.rs` pin and a caller cannot reach.
    //!
    //! A dialect's reader is checked by the shape it parses into, so a test
    //! spells one out and compares. `s3::answer` is a private module of a
