...
fn with_variant(field: &FieldRef, node: &TypePtr) -> (FieldRef, bool) {
    ...
    match field.data_type() {
        ...
        | ArrowDataType::FixedSizeList(child, _) => {
            let [middle] = node.get_fields() else {
                return (Arc::clone(field), false);
            };
            if !middle.is_group() {
                return (Arc::clone(field), false);
            ...
            let dtype = match field.data_type() {
                ArrowDataType::List(_) => ArrowDataType::List(child),
                ArrowDataType::LargeList(_) => ArrowDataType::LargeList(child),
                ArrowDataType::FixedSizeList(_, width) => {
                    ArrowDataType::FixedSizeList(child, *width)
                }
                _ => unreachable!("the list family matched above"),
            };
            (Arc::new(field.as_ref().clone().with_data_type(dtype)), true)
        }
        ArrowDataType::Map(entries, sorted) => {
            ...
            let dtype = ArrowDataType::Map(entries, *sorted);
            (Arc::new(field.as_ref().clone().with_data_type(dtype)), true)
        }
...
///
/// The two known children keep their names, requiredness, and `BYTE_ARRAY`
/// storage, but take neither a logical annotation nor a field id. Shredded
/// children remain as the Arrow converter wrote them.
fn variant_group(field: &ArrowField, ty: &TypePtr, path: &str) -> Result<Type> {
    ...
    let mut value = false;
    let mut fields = Vec::with_capacity(ty.get_fields().len());
    for child in ty.get_fields() {
        match child.name() {
            crate::VARIANT_METADATA_FIELD if !metadata => {
                metadata = true;
                fields.push(variant_child(child, path)?);
            }
            crate::VARIANT_VALUE_FIELD if !value => {
                value = true;
                fields.push(variant_child(child, path)?);
            }
            crate::VARIANT_METADATA_FIELD | crate::VARIANT_VALUE_FIELD => {
                return Err(invalid(
                    path,
                    "one `metadata` and one `value` child for a variant column",
                    "a duplicate child name",
                ));
            }
            _ => fields.push(Arc::clone(child)),
        }
    }
    if !metadata || !value {
        return Err(invalid(
            path,
            "a `metadata` and a `value` child for a variant column",
            "a group missing one",
        ));
    }
    rebuilt_group(
...
/// Rebuild one known variant child without its prohibited field id or
/// annotation, retaining the storage name and requiredness Arrow declared.
fn variant_child(child: &TypePtr, path: &str) -> Result<TypePtr> {
    if !child.is_primitive() || child.get_physical_type() != PhysicalType::BYTE_ARRAY {
        return Err(invalid(
            path,
            "BYTE_ARRAY storage for a variant child",
            storage_name(child),
        ));
    }
    let info = child.get_basic_info();
    let mut builder = Type::primitive_type_builder(child.name(), PhysicalType::BYTE_ARRAY);
    if info.has_repetition() {
        builder = builder.with_repetition(info.repetition());
    }
