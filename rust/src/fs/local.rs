...
#[doc(hidden)]
pub mod internals {
    //! What `rust/tests/fs/local.rs` pins and a caller cannot reach.
    //!
    //! Every destructive operation resolves its path through the same two
    //! private steps, and refusing a volume root - including one spelled as
    //! `.` or reached through `..` - is the whole of the hardening. Neither
    //! is API, so both are reached here; everything the backend does that a
    //! caller can observe is pinned through `yggdryl::` like any other test.
    use std::path::{Path, PathBuf};

    use crate::Result;

    /// Resolve `path` for a destructive `operation`, refusing a volume root.
    pub fn resolve_non_root(path: &str, operation: &'static str) -> Result<PathBuf> {
        super::resolve_non_root(path, operation)
    }

    /// Return whether `path` is a root of a local volume.
    pub fn is_local_root(path: &Path) -> bool {
        super::is_local_root(path)
    }
}
