//! `rust/src/uri/mod.rs`: every identifier, its shared component model, and
//! the invariants a generated corpus holds it to.
                ...
                match Uri::from_path(&input) {
                    Ok(uri) => {
                        *parsed.entry("path").or_default() += 1;
                        ...
                        let canonicalized = input.contains('\\')
                            || input.starts_with("//")
                            || input.starts_with("///");
                        if let Ok(back) = uri.clone().into_path() {
                            ...
                            failures.check(canonicalized || drive_shaped || text == input, || {
                                format!(
                                    "{input:?} -> {uri} -> {text:?} is not the path it started as"
                                )
                            });
                        }
                    }
