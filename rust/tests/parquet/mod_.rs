...
fn an_ascii_field_round_trips_through_the_embedded_arrow_schema() {
    ...
    let schema = media.read_arrow_schema().unwrap();
    let field = yggdryl::Field::from_arrow_field(schema.field_with_name("ccy").unwrap()).unwrap();
    ...
    let stored = media
        ...
        .unwrap()
        .unwrap();
    let text = yggdryl::StructType::from_fields([yggdryl::DataType::utf8().nullable_field("ccy")])
