...
fn apply_arrow_batch_runs_every_protocol_and_walks_nested_declarations() {
    ...
    let applied = root
        .apply_arrow_batch(&events(), true, true, true, true)
        .unwrap();
...
fn apply_arrow_batch_answers_the_same_batch_the_second_time() {
    ...
    assert_eq!(
        root.apply_arrow_batch(&once, true, true, true, true)
            .unwrap(),
        once
    );
}
...
fn apply_arrow_batch_runs_only_the_protocols_it_is_asked_for() {
    ...
    let cast_only = root
        .apply_arrow_batch(&events(), false, false, true, true)
        .unwrap();
    ...
    let partitioned = root
        .apply_arrow_batch(&events(), false, true, true, true)
        .unwrap();
    assert_eq!(
        partitioned
            .column_by_name("top_year")
    ...
    let digested = root
        .apply_arrow_batch(&events(), true, false, false, true)
        .unwrap();
    assert_eq!(
...
fn apply_arrow_batch_refuses_a_root_that_is_not_a_struct() {
    ...
    assert!(
        root.apply_arrow_batch(&events(), false, true, false, true)
            .is_err()
    );
}
...
fn apply_arrow_schema_answers_the_shape_without_reading_a_row() {
    ...
    let applied = root
        .apply_arrow_schema(Arc::clone(&stored), true, true, true, true)
        .unwrap();
    ...
    assert_eq!(
        applied,
        root.apply_arrow_batch(&events(), true, true, true, true)
            .unwrap()
            .schema()
    );
}
...
fn apply_arrow_schema_refuses_a_declaration_the_reader_cannot_satisfy() {
    ...
    let error = root
        .apply_arrow_schema(unrelated, false, true, false, true)
        .unwrap_err()
        .to_string();
...
fn apply_arrow_reader_reports_the_applied_schema_before_the_first_batch() {
    ...
    let mut applied = root
        .apply_arrow_reader(reader, true, true, true, true)
        .unwrap();
...
fn apply_arrow_reader_asked_for_nothing_hands_the_reader_back() {
    ...
    let mut untouched = root
        .apply_arrow_reader(reader, false, false, false, true)
        .unwrap();
