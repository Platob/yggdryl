//! The value stream: one lossless byte stream for any value, and back.

use std::sync::Arc;

use yggdryl::{
    COMPRESS_FROM, DataType, DataTypeId, DataTypeKind, DataTypeValue, Scalar, StringType, TimeUnit,
    Timezone, VALUE_STREAM_VERSION,
};
...
fn the_refusals_name_the_byte_and_the_reason() {
    ...
    let position = Scalar::decode_value_bytes(&[0, DataTypeId::List.as_u8(), 1, 0x2f]).unwrap_err();
