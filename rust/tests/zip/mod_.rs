//! `rust/src/zip/mod.rs`: format records, the index, and the three roles.
//!
//! The format layer is the archive: a caller writes members and reads them
//! back, and never sees a local header, a central record or either
//! end-of-directory trailer. The records this crate does not write - an
//! encrypted member, a method no build decodes, a saturated 32-bit trailer, a
//! name in a code page - have to be stated record by record to be refused or
//! read at all, so those reach `yggdryl::internals`; everything a caller can
//! observe reaches the crate through `yggdryl::`.
...
fn image(members: &[(Entry, Vec<u8>)], comment: &[u8]) -> Vec<u8> {
    ...
    for (entry, encoded) in members {
        let entry = zip_entry::Draft::of(entry.clone())
            .with_header_offset(image.len() as u64)
            .entry();
...
fn a_zip64_trailer_round_trips_its_64_bit_fields() {
    let (directory_offset, directory_size, entries) = (0x1_0000_0000_u64, 46_u64, 70_000_u64);
    let mut trailer = Vec::new();
    format::write_end(directory_offset, directory_size, entries, b"", &mut trailer);

    // The 32-bit record saturates, and the 64-bit pair before it is the truth.
    let ((narrow_offset, _, narrow_entries), _) =
        format::read_end(&trailer[trailer.len() - format::END_LEN..], 0)
            .expect("the 32-bit record");
    assert_eq!(narrow_entries, u64::from(format::ZIP64_MARK_16));
    assert_eq!(narrow_offset, u64::from(format::ZIP64_MARK_32));

    let (wide_offset, _, wide_entries) =
        format::read_zip64_end(&trailer, 0).expect("the 64-bit record");
    assert_eq!(wide_offset, directory_offset);
    assert_eq!(wide_entries, entries);
    let locator = format::read_zip64_locator(&trailer[format::ZIP64_END_LEN..], 0)
        .expect("a locator")
        .expect("the record offset");
    assert_eq!(locator, directory_offset + directory_size);
}
...
fn a_zip64_member_records_its_sizes_in_the_extra_field() {
    ...
    let entry = zip_entry::Draft::new("huge.bin", 0, 0)
        .with_content(1, 0x1_0000_0001, 0x1_0000_0002)
        .entry();
    let mut record = Vec::new();
    format::write_central(&entry, &mut record);
    let read = format::read_central(&record, 0).expect("the record");
...
fn member_names_are_canonical_however_they_were_recorded() {
    let entry = zip_entry::Draft::new("./a//b.txt", 0, 0)
        .with_content(0, 3, 3)
        .entry();
    let root = mounted(image(&[(entry, b"abc".to_vec())], b""));
    assert_eq!(
...
fn a_restart_map_that_does_not_describe_the_member_is_dropped() {
    ...
    let entry = zip_entry::Draft::new("blob.bin", 8, 0)
        .with_content(crc.sum(), encoded.len() as u64, payload.len() as u64)
        .with_restarts(yggdryl::Restarts::new(4 * 1024, vec![64_u64, 128, 192]))
        .entry();
...
fn a_member_name_in_a_code_page_reads_rather_than_failing_the_archive() {
    ...
    let named = zip_entry::Draft::new("cafX.txt", 0, 0)
        .with_content(0, 0, 0)
        .with_flags(0)
        .entry();
    let mut crc = flate2::Crc::new();
    crc.update(b"symbol");
    let plain = zip_entry::Draft::new("ok.txt", 0, 0)
        .with_content(crc.sum(), 6, 6)
        .entry();
...
fn a_rewrite_keeps_what_the_record_said_about_the_member() {
    ...
    let after = root
        ...
        .expect("the index")
        .expect("the member");
    assert_eq!(zip_entry::made_by(&after), zip_entry::made_by(&before));
    assert_eq!(
        zip_entry::external_attributes(&after),
        zip_entry::external_attributes(&before)
    );
}
...
fn a_member_url_opens_the_member_again() {
    ...
    let located =
        yggdryl::zip::from_url(member.url().expect("a member url")).expect("the member reopens");
    ...
    let reopened = yggdryl::zip::from_url(&archive_url).expect("the archive reopens");
...
fn a_member_url_from_a_scheme_this_backend_cannot_hold_is_refused() {
    ...
    let error = yggdryl::zip::from_url(&url).expect_err("a remote archive");
    assert!(matches!(error, Error::Unsupported { .. }), "{error}");
}
...
fn a_member_that_already_carries_a_coding_is_stored_rather_than_recompressed() {
    ...
    member
        .write_all_bytes(&yggdryl::gzip::dump(b"symbol").expect("gzip bytes"))
        .expect("writes");
