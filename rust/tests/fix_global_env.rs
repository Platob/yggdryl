...
//!
//! The environment and the process default are both process-wide, so this
//! binary holds exactly one test and therefore runs on one thread.
...
fn a_malformed_location_errors_and_a_valid_one_settles_the_default() {
    ...
    // A folder holding a shard that does not parse.
    let bad = root.join("bad");
    std::fs::create_dir_all(bad.join("records")).expect("a fresh folder");
    std::fs::write(bad.join("records").join("0.json"), b"not json").expect("a malformed shard");
