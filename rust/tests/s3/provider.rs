//! `rust/src/s3/provider.rs`: which store a location addresses, and the
...
use crate::mod_::{
    BUCKET, file_on, file_on_with, folder_on, options_for, path_on_with, payload, store,
};
...
fn a_container_is_created_and_removed_through_every_store() {
    ...
    for provider in EVERY {
        ...
        let mut folder = yggdryl::s3::folder_with(
            &format!("{}://{name}/", provider.scheme().as_str()),
            options_for(&store, provider),
        )
...
fn a_location_resolves_to_what_it_names_on_every_store() {
    ...
    file_on(&store, Provider::Aws, "lake/part.parquet")
        ...
        .expect("a write");
    for provider in EVERY {
        let leaf = path_on_with(provider, "lake/part.parquet", options_for(&store, provider));
        assert_eq!(leaf.kind(), IOKind::File, "{provider}");

        // A fresh client, so the counters describe this resolution alone.
        let prefix = path_on_with(provider, "lake/", options_for(&store, provider));
...
fn a_location_reports_the_spelling_it_was_handed_on_every_store() {
    ...
    for scheme in ["az", "abfs", "abfss", "wasb", "wasbs"] {
        ...
        let handle = yggdryl::s3::file_with(&spelled, quiet()).expect("an object handle");
    ...
    let keyed = yggdryl::s3::file_with(
    ...
    let azure = yggdryl::s3::file_with(
