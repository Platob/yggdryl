//! `rust/src/s3/properties.rs`: reading a property map written in
...
fn an_encryption_shape_a_store_does_not_have_is_refused_when_the_client_is_built() {
    ...
    let refused = yggdryl::s3::file_with(
        "az://lake/part.bin",
        S3Options::default()
            .with_environment(false)
            .with_azure(yggdryl::s3::AzureOptions::default().with_account("trades"))
