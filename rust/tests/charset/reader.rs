//! `rust/src/charset/reader.rs`: the chunked reader no caller can name.
//!
...
//! handle, so this reaches `yggdryl::internals`. What a caller can observe
//! is beside it here and in `rust/tests/root/charset.rs`.
...
mod internal {
    ...
    use yggdryl::Charset;
    use yggdryl::internals::charset_reader::reader;
    ...
    fn a_transcribing_reader_agrees_with_a_whole_transcription_at_every_split() {
        ...
        for (charset, wire) in [
            ...
            for split in 0..=wire.len() {
                ...
                reader(charset.transcriber(), source)
