//! `rust/src/iceberg/evolve.rs`: evolution and the updates that carry it.
//!
//! Every operation here is one a caller performs on a table's metadata, so the
//! suite is written the way a caller writes one. Only the counters a commit
//! moves are private bookkeeping, and they come through `yggdryl::internals`.

use smol_str::SmolStr;

use yggdryl::iceberg::{
    FormatVersion, PartitionSpec, SchemaUpdate, Snapshot, SnapshotRef, SortField, SortOrder,
    TableMetadata, Transform, assign_field_ids, can_promote,
};
use yggdryl::internals::iceberg_metadata::{
    current_schema_id_mut, default_spec_id_mut, last_column_id_mut, metadata_compression_codec,
    next_row_id_mut, partition_specs_mut, schemas_mut, sort_orders_mut,
};
...
mod schema_updates {
    ...
    fn schema_ids_come_from_the_official_result_and_overflow_is_atomic() {
        let mut metadata = metadata();
        schemas_mut(&mut metadata)[0]
            .as_iceberg_mut()
            .set_schema_id(i32::MAX)
            .unwrap();
        *current_schema_id_mut(&mut metadata) = i32::MAX;
...
mod metadata_updates {
    ...
    fn metadata_compression_consults_only_its_official_property() {
        ...
        metadata
            ...
            .unwrap();
        assert_eq!(
            metadata_compression_codec(&metadata).unwrap(),
        ...
        metadata
            ...
            .unwrap();
        let message = metadata_compression_codec(&metadata)
            .unwrap_err()
            .to_string();
        assert!(
    ...
    fn upgrading_to_v3_initializes_next_row_id_and_a_downgrade_is_refused() {
        ...
        *next_row_id_mut(&mut missing) = None;
        ...
        *next_row_id_mut(&mut negative) = Some(-1);
    ...
    fn explicit_expiration_protects_heads_and_trims_metadata() {
        ...
        assert_eq!(metadata.snapshot_log(), [(timestamp + 1, 2)]);
        assert!(metadata.statistics().is_empty());
    ...
    fn add_spec_reuses_equivalent_specs_and_renumbers_new_ones() {
        ...
        different.fields[0].transform = Transform::Bucket(8);
        ...
        partition_specs_mut(&mut exhausted)[0].spec_id = i32::MAX;
        *default_spec_id_mut(&mut exhausted) = i32::MAX;
        assert_eq!(
    ...
    fn add_sort_order_preflights_the_official_identifier_increment() {
        let mut metadata = metadata();
        sort_orders_mut(&mut metadata).push(identity_order(i64::MAX));
    ...
    fn validate_catches_a_duplicate_field_id_and_a_stale_last_column_id() {
        ...
        schema
            ...
            .unwrap();
        schemas_mut(&mut duplicated)[0] = schema;
        ...
        *last_column_id_mut(&mut stale) = 2;
