//! `rust/src/iceberg/catalog/mod.rs`: catalogs of namespaces of tables.
//!
//! Every level of the hierarchy is one shape a caller holds, so the whole
//! suite reaches the crate through `yggdryl::iceberg` and nothing else.
...
fn names(names: Names) -> Vec<String> {
    names.collect::<yggdryl::Result<Vec<_>>>().unwrap()
}
...
fn the_cascade_and_the_dotted_name_reach_the_same_table() {
    ...
    assert_eq!(
        cascaded.metadata().table_uuid(),
        dotted.metadata().table_uuid()
    );
    assert_eq!(
        cascaded.metadata().table_uuid(),
        entry.metadata().table_uuid()
    );
...
fn a_catalog_names_itself_after_its_warehouse_folder() {
    ...
    let memory = Catalog::new(yggdryl::holder::Buffer::new());
    assert_eq!(
        memory.name(),
        memory.warehouse().url().and_then(yggdryl::Url::file_name)
    );
}
