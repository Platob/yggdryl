//! `rust/src/iceberg/snapshot.rs`: branches, tags, and what they retain.
//!
//! A reference is what a reader resolves and expiration prunes against, so
//! these pin the moving, the renaming, the fast-forward and the retention
//! arithmetic through `yggdryl::iceberg`. Only the commit clock is out of a
//! caller's reach, and it comes through `yggdryl::internals`.

use smol_str::SmolStr;

use yggdryl::iceberg::{
    FormatVersion, MAIN_BRANCH, PartitionSpec, Snapshot, SnapshotRef, TableMetadata,
    assign_field_ids,
};
use yggdryl::internals::iceberg_metadata::{now_ms, refs_mut, snapshot_log_mut, snapshots_mut};
...
fn chained(count: i64, step_ms: i64) -> TableMetadata {
    ...
    for snapshot in snapshots_mut(&mut metadata) {
        snapshot.timestamp_ms = now - (count - snapshot.snapshot_id + 1) * step_ms;
    }
    for (timestamp, snapshot_id) in snapshot_log_mut(&mut metadata) {
...
mod expiration {
    use super::{MAIN_BRANCH, SnapshotRef, chained, now_ms, snapshots_mut, table};
    use yggdryl::Scalar;
    ...
    fn the_official_default_ref_age_removes_a_stale_tag() {
        let mut metadata = chained(3, 100_000);
        snapshots_mut(&mut metadata)[2].parent_snapshot_id = None;
        metadata
    ...
    fn expiring_snapshots_removes_both_statistics_kinds() {
        ...
        for snapshot_id in [1_i64, 2] {
            let statistics = yggdryl::json::from_utf8(&format!(
            ...
            let partition_statistics = yggdryl::json::from_utf8(&format!(
...
mod validation {
    use super::{SmolStr, SnapshotRef, chained, refs_mut};
    ...
    fn validate_names_a_tag_carrying_a_branch_retention_field() {
        ...
        refs_mut(&mut metadata).last_mut().unwrap().1 = SnapshotRef {
            snapshot_id: 1,
            kind: SmolStr::new_static("tag"),
...
mod strict_json_tests {
    ...
    use yggdryl::internals::iceberg_snapshot::validate_for_version;
