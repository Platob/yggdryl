//! `rust/src/media/partition.rs`: partitions move between path and data.
//!
//! Everything a caller can observe reaches the crate through `yggdryl::`. The
//! reader a partitioned tree is read through is crate-private, and that it
//! neither pulls the listing nor opens a leaf before the batch that needs it
//! is a claim only a counting folder handed straight to it can make, so
//! `lazy_folder` reaches `yggdryl::internals`.
