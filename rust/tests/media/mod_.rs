//! `rust/src/media/mod.rs`: the one value naming every record encoding,
//! exercised through the one surface they share.
...
fn a_compressed_handle_mirrors_the_compressed_bytes() {
    ...
    assert_eq!(media.read_range_bytes(0, 2).unwrap(), [0x1F, 0x8B]);
}
