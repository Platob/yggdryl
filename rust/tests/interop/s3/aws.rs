...
fn object(key: &str) -> yggdryl::s3::S3File {
    yggdryl::s3::file_at_with(Provider::Aws, BUCKET, key, options()).expect("an object handle")
}

/// The prefix `key` names in the exchange bucket.
fn prefix(key: &str) -> yggdryl::s3::S3Folder {
    yggdryl::s3::folder_at_with(Provider::Aws, BUCKET, key, options()).expect("a prefix handle")
}
...
fn objects_written_here_are_readable_here_and_by_boto3() {
    ...
    let mut big = yggdryl::s3::file_at_with(
        Provider::Aws,
        BUCKET,
...
fn the_encryption_headers_are_what_botocore_computes() {
    let customer = yggdryl::s3::Encryption::customer(&exchange_key()).expect("a 32-byte key");
    for (name, value) in customer.write_headers(Provider::Aws) {
        println!("SSE-C {name} {value}");
    }

    let kms = yggdryl::s3::Encryption::Kms(
        yggdryl::s3::KmsKey::new(KMS_KEY_ID)
