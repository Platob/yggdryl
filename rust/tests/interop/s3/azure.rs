...
fn account() -> String {
    std::env::var("AZURE_STORAGE_ACCOUNT_NAME")
        .unwrap_or_else(|_| yggdryl::s3::azure::DEVELOPMENT_ACCOUNT.to_owned())
}

/// The shared key it is signed with.
fn key() -> String {
    std::env::var("AZURE_STORAGE_ACCOUNT_KEY")
        .unwrap_or_else(|_| yggdryl::s3::azure::DEVELOPMENT_KEY.to_owned())
}
...
fn prefix(key: &str) -> yggdryl::s3::S3Folder {
    yggdryl::s3::folder_at_with(Provider::Azure, CONTAINER, key, options())
...
fn container() -> yggdryl::s3::S3Folder {
    let root = yggdryl::s3::folder_at_with(Provider::Azure, CONTAINER, "", options())
...
fn a_large_blob_is_staged_as_blocks_and_committed_as_a_list() {
    ...
    let bytes: Vec<u8> = (0..9 * 1024 * 1024)
        ...
        .collect();
    let mut handle = yggdryl::s3::file_at_with(
        Provider::Azure,
        CONTAINER,
