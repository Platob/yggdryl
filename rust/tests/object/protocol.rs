...
use yggdryl::object::{AwsOptions, Credentials, ObjectOptions};
...
use crate::mod_::{BUCKET, file, file_with, folder, folder_with, options, path, payload, store};
...
fn a_location_naming_no_container_is_refused_before_anything_is_built() {
    let error = yggdryl::object::file("file:///tmp/part.parquet").expect_err("a refusal");
    ...
    yggdryl::object::folder("https://example.com/x").expect_err("a refusal");
    // A store's own word for it is what its own refusal uses: a location that
    // names only an endpoint has named no container yet.
    let error = yggdryl::object::file("gs://storage.googleapis.com/").expect_err("a refusal");
    assert!(error.to_string().contains("naming a bucket"), "{error}");
    let error = yggdryl::object::file("az://trades.blob.core.windows.net/").expect_err("a refusal");
    assert!(error.to_string().contains("naming a container"), "{error}");
}
...
fn every_s3_url_spelling_reaches_the_same_object() {
    ...
    for scheme in ["s3", "s3a", "s3n"] {
        ...
        let mut handle =
            yggdryl::object::file_with(&url, options(&store)).expect("an object handle");
        ...
        let listed = yggdryl::object::folder_with(&prefix, options(&store))
        ...
        assert_eq!(
            yggdryl::object::located_with(&url, options(&store))
                .expect("a location")
                .kind(),
            IOKind::File
        );
    ...
    for scheme in ["s3", "s3a", "s3n"] {
        let url = format!("{scheme}://{BUCKET}/lake/{scheme}.bin");
        let refused = yggdryl::object::file_with(&url, options(&store))
...
fn a_named_role_is_traded_for_a_session_once_and_signs_everything_after() {
    ...
    let role = yggdryl::object::AssumedRole::new("arn:aws:iam::123456789012:role/lake-reader")
