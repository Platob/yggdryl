//! `rust/src/digest.rs`: the `DigestAlgorithm` vocabulary, the `Digest` value
//! it names, and the digester that answers the same bytes a one-shot does.
...
fn the_digester_is_a_hasher() {
    ...
    assert_eq!(digester.finish(), yggdryl::xxhash::xxh3(b"abc"));

    let mut wide = DigestAlgorithm::Xxh128.digester();
    wide.write(b"abc");
    assert_eq!(wide.finish(), yggdryl::xxhash::xxh3(b"abc"));
}
