//! `rust/src/txhash/arrow.rs`: the time-column predicate no caller can name.
//!
//! `accepts_time` is the crate-private rule the column reader branches on, so
//! the two agreeing is the contract, and it is reached through
//! `yggdryl::internals`. Everything a caller can observe lives beside it here.

#[cfg(feature = "internals")]
mod internal {
    use yggdryl::internals::txhash_arrow::accepts_time;
    use yggdryl::txhash::arrow::unix_array;
...
mod columns {
    ...
    fn compose_and_decompose_are_inverses_at_every_width() {
        ...
        for algorithm in DigestAlgorithm::ALL {
            ...
            let (instants, split) =
                decompose(coupled.as_ref(), TimeUnit::Second, algorithm).unwrap();
            assert_eq!(
                instants.data_type(),
            ...
            match algorithm {
                ...
                // `DigestAlgorithm` is non-exhaustive outside the crate; a new
                // one reaching here has no column reading stated yet.
                other => panic!("{other} has no coupled column reading"),
            }
            assert_eq!(
    ...
    fn a_dotted_time_path_reads_an_instant_under_a_nested_struct() {
        let inner =
            DataType::from(StructType::from_fields([event_field(), symbol_field()]).unwrap());
    ...
    fn a_containing_holder_reads_a_nested_coupled_holder_as_its_bytes() {
        ...
        let expected =
            Scalar::from_sequence([inner_value.into_scalar()]).digest(DigestAlgorithm::Xxh3);
        assert_eq!(
