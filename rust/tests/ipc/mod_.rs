//! `rust/src/ipc/mod.rs`: arrow IPC streams over any byte handle.
...
mod internal {
    ...
    use yggdryl::ipc::Ipc;
    use yggdryl::{DataType, Field, IOBase, IOMedia, StructType, Url};
    ...
    fn a_closed_stream_fetches_fresh_and_an_open_one_holds_what_it_cached() {
        ...
        handle_behind_the_cache(&mut probe)
...
mod records {
    ...
    use yggdryl::{Codec, DataType, Field, StructType, Url};
    ...
    fn multi_batch_reader() -> yggdryl::arrow::BatchReader {
        let first = batch();
    ...
    fn empty_reader() -> yggdryl::arrow::BatchReader {
        empty_reader_for(&schema())
    }

    /// A reader that declares `field` and yields nothing.
    fn empty_reader_for(field: &Field) -> yggdryl::arrow::BatchReader {
        yggdryl::arrow::batch_reader(field.clone().into_arrow_schema().unwrap(), [])
    }
    ...
    impl IOBase for Counting {
        ...
        fn pread(&self, offset: u64, bytes: &mut [u8]) -> yggdryl::Result<usize> {
    ...
    fn an_open_cache_tracks_selection_and_completion_on_overwrite() {
        ...
        yggdryl::IOMedia::overwrite_arrow_reader(&mut media, reader(), &options).unwrap();
    ...
    fn schema_reads_bypass_buffer_pages_and_stop_before_the_batch_body() {
        ...
        let narrow = StructType::from_fields([DataType::Int64.required_field("id")])
            .map(DataType::from)
            .unwrap()
            .required_field("row");
    ...
    fn compressed_schema_reads_stop_before_the_encoded_batch_body() {
        ...
        assert!(total > yggdryl::DEFAULT_STREAM_BATCH_SIZE);
    ...
    fn an_absent_compressed_stream_keeps_its_declared_empty_schema() {
        ...
        assert_eq!(media.row_size().unwrap(), 0);
        assert_eq!(batches.schema(), schema().into_arrow_schema().unwrap());
    ...
    mod limits {
        ...
        fn rows(handle: &Buffer, options: &yggdryl::media::RecordOptions) -> usize {
            handle
                .read_arrow_reader(options)
                .unwrap()
                .map(|batch| batch.unwrap().num_rows())
        ...
        fn a_limit_counts_result_rows_after_projection_and_cast() {
            ...
            let declared = StructType::from_fields([DataType::utf8().required_field("id")])
                .map(DataType::from)
                .unwrap()
                .required_field("row");
            let options = handle
