...
fn registering_a_message_type_names_it_describes_it_and_never_rewrites_it() {
    let mut registry =
        FixRegistry::from_fields([tagged("msgtype", yggdryl::fix::MSGTYPE_TAG_NAME.0)]).unwrap();
...
fn group_entry_names_singularize_published_collections() {
    use yggdryl::internals::fix_component::{entry_name, group_name};
...
fn every_type_adopted_backward_parses_the_wire_spelling_of_its_era() {
    ...
    for (tag, spelling, wire, dtype) in [
        ...
        (
            ...
            DataType::datetime64(yggdryl::TimeUnit::Nanosecond, yggdryl::Timezone::UTC).unwrap(),
        ),
        (
            9_004,
            "TZTimeOnly",
        ...
        (
            ...
            "LocalMktDate",
            "20240102",
            DataType::datetime64(yggdryl::TimeUnit::Nanosecond, yggdryl::Timezone::NAIVE).unwrap(),
        ),
    ] {
...
fn the_specifications_retirements_restate_as_documents_of_the_same_rules_would() {
    ...
    for value in fields {
        ...
        stripped.push(yggdryl::internals::fix_document::dump(field.clone().into_value()).unwrap());
