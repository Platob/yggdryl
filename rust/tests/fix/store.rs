...
fn code_sets_round_trip_through_their_own_folder_and_are_pruned_when_they_go() {
    ...
    let loaded = FixRegistry::from_handle(&folder).unwrap();
    assert_eq!(loaded, registry);
    assert_eq!(loaded.codesets().len(), 2);
    assert_eq!(
...
fn enum_codes_belong_to_each_field() {
    ...
    // Two vocabularies, held once each under the name the field states -
    // and neither is a category: a category holds Field documents and
    // resolves references between them, which is why `codesets` is written
    // beside the three folders rather than as a fourth.
...
fn a_document_property_is_stored_as_the_json_it_is_and_read_back_as_its_text() {
    // A field's `FIX:codes` names the set it reads by and is one text, so
