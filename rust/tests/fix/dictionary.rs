...
/// row hashes one member fewer. It last moved when a field's vocabulary
/// became a named code set: the members left the field's own `FIX:codes`,
/// which now holds the set's name, and the dictionary hashes each set once
/// under that name beside the fields, so every coded field's metadata and
/// the whole of the shipped vocabulary changed what they contribute.
#[test]
fn the_committed_dictionary_hashes_to_one_pinned_value() {
    let registry = seed();
    assert_eq!(registry.stable_hash(), 16_437_756_573_517_745_290);
