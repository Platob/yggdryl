...
fn native_mapping_survives_message_rows_and_arrow_in_both_directions() {
    ...
    for pairs in [
        ...
        let row = Scalar::from_sequence([mapping]);
...
fn canonical_map_names_win_over_scalar_aliases_for_reads_writes_and_paths() {
    ...
    let mut message = FixMsg::from_row(
        Arc::clone(&registry),
        &schema,
        &Scalar::from_sequence([Scalar::from("stated-label"), mapping("O-1")]),
    )
    .unwrap();
    ...
    assert_eq!(message.as_field().fields().len(), 2);
}
