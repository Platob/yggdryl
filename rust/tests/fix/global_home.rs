...
//! all process-wide, so the layer binary runs this case in its own selected
//! child process. The home it points at is a fresh directory of its own,
...
fn the_configuration_directory_seeds_the_default() {
    ...
    assert!(home.join(".config/fix/primitive/standard/0.json").is_file());
