//! `rust/src/holder/buffered/mod.rs`: what the page cache does, counted rather
//! than assumed.
...
mod counted {
    ...
    use yggdryl::{IOBase, IOMedia, MediaType, MimeType, Url};
    ...
    fn a_url_named_handle_keeps_its_own_identity() {
        let inner = Buffer::new()
            .with_media_type(Url::from_str("file:///trades.arrows").unwrap().media_type());
