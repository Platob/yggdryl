...
//! [`local::LocalFile`](yggdryl::local::LocalFile), the memory-mapped local
//! backend, on the same payload.

use std::hint::black_box;

use criterion::{Criterion, Throughput};
use yggdryl::IOBase;
use yggdryl::fs::FsFile;
...
pub(crate) fn byte_benchmarks(criterion: &mut Criterion) {
    ...
    group.bench_function("read_all/local_file", |bencher| {
        ...
        let mut handle = yggdryl::local::LocalFile::create(&path).expect("a valid path");
        handle.write_all_bytes(&bytes).expect("the fixture writes");
