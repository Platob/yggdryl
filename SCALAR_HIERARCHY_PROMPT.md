...
**A number attaches directly** — `Int8`, `Float16`, `Decimal128`,
...
- **`Integer` has ten members where `IntegerType` has eight.** Arrow has no
  ...
  size is hot.
- **`Ascii` names the family and one of its members.** The ASCII family holds
  the variable form `Ascii`, the parameterized `FixedAscii(i32)`, and the four
  registered codes. The variable form has the family's own name, which the
  *no family name is a member name* rule otherwise forbids - it is admitted
  here because `ascii` is what the grammar spells the unparameterized shape as
  and `ascii(n)` its width, so any other member name would be a second spelling
  of a datatype that already has one. Inside `AsciiType` the member is
  unqualified; the family enum is reached as `AsciiType`, never as `Ascii`.
...
| `Ascii` | 6 | `Ascii` `FixedAscii` `Country` `Currency` `Mic` `Cfi` |
