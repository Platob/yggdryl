...
| end of a handle or folder leaf | framing state ends, so records never join across source objects; each leaf is read under its own identifier and so supplies its own `sourceurl` / `crosscode`, and numbering restarts |
