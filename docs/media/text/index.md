...
four of whose captures name the [FIX columns they fill](../../fix/arrow.md#a-bridge-log-names-what-it-fills)
when the read goes on into a FIX batch; its `timestamp`, `msgthreadid` and
`level` name none and are carried in front as the capture's own columns, so
that header declares no `mtime` capture and does not date its own lines.
...
A line the header did not match and a line whose capture will not parse are not the same fact, and the reader answers them differently. An unmatched line states nothing: every capture is null, the line keeps its body, and it settles at the handle's clock or the epoch — a header that matches no line at all therefore reads a whole capture without a word of complaint. A capture that *did* match and then does not read as the fact it names is a refusal located on the line and the column, and the fallible doors — `TextLine::mtime` among them, which is what the FIX codec calls for a row's `recdunix` — return it rather than a value, so it ends the read. The reasoning is that the two say different things: an unmatched line is a line this header was not written for, while a matched one that will not parse is the header's own claim about the text turning out to be false, and silently nulling that hides a misconfigured capture across every row of a capture. The consequence to hold on to is that **widening a row header can turn a line it used to ignore into a line that ends the read**: the text it now matches has to parse as well. Widening it also changes which lines carry their captures at all, which is a [lifecycle](../../fix/lifecycle.md) consequence rather than a reading one.
...
- a `state`, `prevuuid`, or instant capture the line cannot read at the fact's datatype -> a refusal naming the row, the object and the fact, on the batch as on the reading; it ends the read there, and neither `safe` - which governs a cast, not a capture's reading - nor `autotype` degrades it, because these captures are read at their fact's datatype whatever the expression's syntax suggests. A line the header did not match is the other case entirely and keeps its body, so widening a header can make a line it used to ignore end the read.
