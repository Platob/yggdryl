...
| Row | the [row schema](#row-schema): the sixteen [event columns](../../graph.md#columns), then `sourceurl`, `rownum`, `mtime`, `mimetype`, `body`, `dropped_byte_size`, then every row-header capture and lifted entry |
...
The source field is complete before any source bytes are read. It opens with the sixteen [event columns](../../graph.md#columns) every event's schema opens with - a line is an [event](lines.md#lines) of the graph, and the FIX row parsed out of it opens with the same sixteen under the same names, so a message's `srcuuids` joins the line's `curruuid` without a mapping - then the line's own columns.
