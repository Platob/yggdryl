...
| [Market](market/index.md) | `market/`: the five products the market's messages state - `order.rs`, `quote.rs`, `execution.rs`, `trade.rs`, `book.rs`, each a trait beside its holder and a graph event with its own row - `symbol.rs` the key an instrument's books are read under, `statement.rs` one value over the four that state something to a ladder, `iterator.rs` the book iterator, and `product.rs` for what they share: the market columns, the `Product` trait, the fold, the walk and the row doors; their FIX reading is `fix/market.rs` |
...
| Rule | Consequence |
| --- | --- |
| A schema is a field | A non-null Struct [`Field`](types/field.md) is the only row schema; `Scalar::Struct` is named input that canonicalizes to an ordered `Scalar::Sequence`. |
