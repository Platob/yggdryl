...
| [Media: JSON, YAML, TOML](media/index.md#json) | `json/`, `toml/`, `yaml/`: structured `Scalar` codecs over the machinery in `text/`, three more sections of the [Media](media/index.md) page |
...
| [Graph](graph.md) | `graph/element.rs`: the `Element` and `Event` traits - an element's `Uuid`, the identity it has elsewhere, its sources' UUIDs, an event's instant, state and place in its chain - as signatures a value implements; `graph/market.rs` the `Market` and `MarketOperation` traits - the instrument, the side, the price and the quantity, then the operation's category, time in force, identifier maps and lanes - and their `MarketEvent` and `MarketOperationEvent` blankets; `graph/event.rs` the four holders, `graph/operation.rs` the one `Operation` and its book control, `graph/book.rs` the books and `graph/iterator.rs` the one walk |
...
| Text speaks values | JSON, YAML, and TOML parse and render one [`Scalar`](media/index.md#json); the exact field directs nullability, order, and dictionaries. |
