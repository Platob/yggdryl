...
| Crate definitions | The [crate listing](capture.md#the-crates-own-columns) has 38 definitions from tag 65003: 36 scalar fields and the sorted Map groups `identifiers(65020)` and `metadata(65049)`. `new()` registers every one, beside its own `SendingTime(52)` and `TransactTime(60)`, so an empty registry holds 38 fields and two groups and its `len()` is 40. A [store](store.md) writes these builtins like any other definition, and a stored one can never override the constructed one |
