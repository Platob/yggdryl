...
| Validated | at insert, update and load alike, the registry parses the text and checks the grammar's depth and node budget, refusing a malformed one by the field's name; whether every name is a field or a group of the registry, and whether the term types against those fields, is proven once when the derivations compile - on the first enrichment or row fill - refusing by the field's name and reading no message for it |
...
| Typed | the answer lands through the dictionary's own field for the tag, so `then 0` on a `float64` field is `0.0`, `then '2'` on a `state` field is the state `2` spells, and a `try_cast(... as country)` that answers `XS` is refused by nothing but the term's own conditions |
...
| `OrderQty(38)` | `case when msgtype in ('8', '9') then coalesce(cumqty + leavesqty, cumqty + cxlqty) end` |
...
| `CountryOfIssue(470)` | `case when substring(isincode, 1, 1) <> 'X' and not substring(isincode, 1, 2) between 'QM' and 'QZ' and not substring(isincode, 1, 2) in ('AA', 'AN', 'CS', 'EU', 'YU', 'ZZ') then try_cast(substring(isincode, 1, 2) as country) end` |
...
| Fills | `fix:derivation` |
| --- | --- |
| `isincode` | `coalesce(case when securityidsource = '4' then securityid end, secaltidgrp[securityaltidsource = '4'][0].securityaltid)` |
