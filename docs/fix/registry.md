...
`merge_with` on a field unions the two lists; `FixRegistry::merge_with`, `add_fields` and `add_cfb` carry that union onto whatever the registry already held, so a merged registry says which dictionaries spoke each field.
...
`merge_with` combines another registry under the [fold table](#what-one-namespace-means-for-a-field-that-arrives), its code sets folded first, its named definitions folded member by member and each field's membership unioned - the sets lead because a field keeps the set it already reads by, so the members the other dictionary states have to be in that set by the time the field is folded, and a merge therefore widens a vocabulary and never narrows one; `add_fields` folds a scalar field iterable the same way; `add_cfb(location, dialect)` parses every CBlock one location names and merges them, stamping the dialect - or, with none supplied, each file's own stem where it reads as a name, opening with a letter - on everything that file produced; a supplied name that is empty or carries a comma is refused.

**One argument says which files, and the reader works out which kind it is.** A `location` holding `*`, `?` or `[` is a pattern, anchored the way `IOBase::glob` anchors one - a fixed prefix is descended rather than listed and filtered, `**` spans any number of levels - and what it matched is the answer. A location naming a folder is every `.cfb` beneath it at any depth, which is what pointing at a tree of counterparty files nested by venue is for. Anything else is one file. Private entries are never matched, a pattern selecting nothing folds nothing, and a *fixed* location naming nothing is refused rather than folding an empty dictionary that would read as a success. So `cblocks`, `cblocks/BLPFIX44.cfb`, `cblocks/*.cfb` and `**/venue-*.cfb` are four spellings of one door rather than two doors and a split the caller has to make; in Rust the container is the first argument and the location the second, because a `*` cannot survive inside a handle's URL, while Python takes the one location and makes that split itself.

The dialect is resolved per file, so `cblocks/*.cfb` with none supplied stamps `msfix44` and `blpfix44` from the two files' own stems. Files fold in **ascending URL order** whatever order the listing arrived in, because the fold's precedence is its input order and a glob's sequence varies with how the pattern decomposed and with the backend beneath; so where two files disagree about one tag the last-sorting file wins, and `cblocks`, `cblocks/*.cfb`, `cblocks/**/*.cfb` and `**/venue-*.cfb` over the same files all answer the same dictionary. It answers `(files, added, merged)` - the file count is a fact only this call holds, since an empty match and a match whose files all merged into stored fields both answer zeroes for the other two. `add_json(location)` is the same door for a [JSON snapshot](store.md), reading `.json` beneath a folder, and takes no dialect, because a snapshot is the crate's own format and every field and definition in it already carries the `FIX:branches` its writer meant.

These operations report their counts only after the entire staged fold succeeds, and a plural one pays one copy of the dictionary for the whole call rather than one per file: a file that will not parse leaves the dictionary exactly as it was and the refusal names that file among however many matched.
