...
| one of the eighteen [event columns](../graph.md#columns) | nothing: they are the carrier's own facts - the [text line](../media/index.md#plain-text) each row is, as the reader stated it - so `curruuid` is each message's one source and the rest fill no message fact |
