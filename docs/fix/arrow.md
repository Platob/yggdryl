...
`write_fix` streams a batch back out as frames, rebuilt from each row's `entries` and never from its columns: the fixed columns are a *reading* of the message, so a frame rebuilt from them would be one nobody sent. A batch carrying no `entries` column cannot be written and says so plainly. One batch is pulled, its rows written, and it is dropped; no buffer bigger than a row is held. Rust only.
