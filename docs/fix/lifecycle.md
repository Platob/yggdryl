...
=== "Rust"
    ...
    // A message descends from the whole chain before it. Ancestry is a
    // canonical set: sorted by identity, never a chronology encoded by position.
    let mut ancestry = [order.get_curruuid(), ack.get_curruuid()];
    ancestry.sort_unstable();
    assert_eq!(fill.get_parentuuids(), ancestry);
...
=== "Python"
    ...
    # A message descends from the whole chain before it. Ancestry is a
    # canonical set: sorted by identity, never a chronology encoded by position.
    assert len(fill.parentuuids) == 2
    assert order.curruuid in fill.parentuuids
    assert ack.curruuid in fill.parentuuids
    # The lifecycle travels: the chain's first creation, and the state.
...
=== "JavaScript"
    ...
    // A message descends from the whole chain before it. Ancestry is a
    // canonical set: sorted by identity, never a chronology encoded by position.
    assert.equal(fill.parentuuids.length, 2)
    assert.ok(fill.parentuuids.includes(order.curruuid))
    assert.ok(fill.parentuuids.includes(ack.curruuid))
    // The lifecycle travels: the chain's first creation, and the state.
