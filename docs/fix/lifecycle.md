...
| Follows | the predecessor's `curruuid` and `currunix` recorded, `seqnum` one past the predecessor's, the predecessor's whole lineage as its parents, oldest first, the chain's cross code forced, the names the predecessor went by taken, and the lifecycle carried: the earliest `creaunix`, the newer message's explicit `exprtime` where it states one else the predecessor's, and the furthest [state](../types/codes/state.md#the-rank-leads). A replacement may shorten as well as extend its deadline. Merging two statements of one event still keeps the latest expiry either statement knows. A message that moved is settled again, so its `currhashcode` and `curruuid` are its own |
