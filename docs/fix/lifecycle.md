...
A bridge logs one message at every hop it passes, so a capture routinely holds the same delivery twice. `lifecycle` removes an exact republication or true retransmission across the whole finite capture, even after more than 4,096 distinct deliveries: a complete FIX header keys the session, sequence, original time and content, while a headerless bridge row can prove only an exact repeated event in the same capture session, context and direction. Different sessions, contexts and sequence numbers remain different deliveries. The retained delivery set therefore grows with distinct deliveries in that finite capture; it is not a recent window.
