...
| Follows | before following, equal complete `msgsesseventid` values force a full content-and-graph merge: the latest recorded observation is the reference, while `recdunix` and `execunix` become the earliest values either statement knows. Otherwise the predecessor's `curruuid` and `currunix` are recorded, `seqnum` is one past the predecessor's, the predecessor's whole lineage becomes its parents oldest first, the chain's cross code and prior names carry, and so do the earliest `creaunix`, the newer message's explicit `exprtime` where stated else the predecessor's, the furthest [state](../types/codes/state.md#the-rank-leads), and the later of the predecessor's latest execution clock and this event's precise `execunix`. `recdunix` and `refrecdunix` remain this observation's own. A missing `parentorderid` takes a different predecessor `OrderID(37)`, and a missing `ClOrdID(11)` takes the predecessor's; an explicitly stated link always wins. A full merge persists its selected latest recording as `refrecdunix`, so another merge selects the same reference after `recdunix` has become the earliest observation. A message that moved is settled again, so its `currhashcode` and `curruuid` are its own |
...
`srcuuids` keeps the reference first and the statements folded into it after, each once. Only that first entry is specified. Which observation becomes the reference is settled by the greatest `refrecdunix`/`recdunix` and then the later `currunix`, and where those tie the order the caller handed the observations in decides it - so feeding one capture in the other order answers the same provenance set spelled in a different order. A join may read the head as the statement whose conflicts won, and the set as the whole provenance, and must not read the tail as arrival order. A missing or empty text part or missing sequence produces no `msgsesseventid`, and a different message type, context, session or sequence is a different delivery. Already placed lifecycle outputs and snapshot views are not raw capture observations and are never coalesced again on replay.
