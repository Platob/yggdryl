...
| Compiled | once per registry, on the first enrichment or row fill, and kept until a field or a definition changes: every term parsed, every name it reads proven a field or a group of the registry, the term typed against the registry's own fields; a name the dictionary lacks or a term that does not type refuses at that first call, naming the field, never per message |
| Bound | once per distinct root shape: the message's root widened with the columns the derivations read or fill that it lacks, each typed by the registry's field, and every term bound against that widened root; a stream of a million same-shaped messages binds once, and a small bounded cache of shapes serves every codec and every row fill over the registry |
| Swept | in tag order over a working row: a target the row holds non-null is skipped; else the term is evaluated, a null answer is silence, a non-null one is typed by the target's field exactly as `set` types a value - a value the field refuses is silence - and written where the next derivation reads it |
...
| Crate columns | `isincode`, `miccode` and `state` derive by the same evaluator - on the message when enriched, on the row when `into_row` is asked for a column the message does not state - and their terms read the standard's fields by name, so a registry built from a handful of fields widens what it lacks as null inputs rather than refusing to enrich |
| Silence | an absent input, a condition that does not hold, an answer the field refuses, an evaluation the grammar refuses, and a shape a term cannot bind against are all a null column and never a failure |
...
| `CountryOfIssue(470)` | `isincode` | ISO 6166 opens a number with the ISO 3166 code of the numbering agency's country, where it is one: the user-assigned range (`AA`, `QM`-`QZ`, `XA`-`XZ`, `ZZ`), the transitionally reserved `AN`, `CS`, `YU` and the reserved `EU` are what the crate's own country registry excludes, so `XS` and `EU` answer nothing |
...
| `LeavesQty(151)`, `OrderQty(38)`, `CumQty(14)` | the other two, on a report | Appendix D: `OrderQty = CumQty + LeavesQty`, nothing is left once `OrdStatus(39)` is closed, and what a canceled order asked for is what it did plus `CxlQty(84)` where nothing says what is left |
...
- A derivation the dictionary cannot compile refuses the first enrichment, naming the field, and reads no message for it: `lastqty * nosuchfield` on `GrossTradeAmt` names a column no field or group of the registry answers to, and `lastqty * symbol` does not type. A text that is not a term is refused earlier still, by the registry, at insert, update and load.
- A registry built from a handful of fields enriches: the crate columns' own derivations read the standard's fields by name, and a name such a registry lacks is an input it never states, widened as null, rather than a refusal.
