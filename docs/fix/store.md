...
| Snapshots | `FixRegistry::into_json`/`from_json` render and read the whole catalog in that same shape, the `codesets` array leading the three category arrays; `add_json(location)` folds every such file one location names - a glob, a folder of `.json` read at any depth, or one file - into a dictionary the way `merge_with` folds any, answering `(files, added, merged)`, and takes no dialect - a snapshot already carries the `FIX:branches` its writer meant |
...
=== "Rust"
    ...
    // 30 scalar fields, including two UUID lists, and two Map groups.
    assert_eq!(fix_crate_fields()?.len(), 32);
