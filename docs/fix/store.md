...
=== "Rust"
    ...
    assert_eq!(registry.codesets().len(), 735);
...
=== "Python"
    ...
    assert len(registry.codeset_names()) == 735
...
=== "JavaScript"
    ...
    // The vocabularies are held beside them, one per name, and a field
    // reaches its own through the name it states.
    assert.equal(registry.codesetNames().length, 735)
