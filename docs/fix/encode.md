...
the FIX fields the message lifted onto that holder, the [entries](message.md) -
the content row read as a tree, in its order - then the standard trailer, each
value as the wire spells it.
...
=== "Rust"
    ...
    // The header from its holder, then the fields the message lifted - the
    // quantity it is about - then the content with the side as its wire
    // code, the day order the dictionary derived for it, and the trailer.
    assert_eq!(
        emitted,
        "8=FIX.4.4|35=D|52=20260102-10:15:30|38=100|55=AAPL|54=1|59=0|10=000|",
    );
...
=== "Python"
    ...
    # The header from its holder, then the fields the message lifted - the
    # quantity it is about - then the content with the side as its wire code,
    # the day order the dictionary derived for it, and the trailer.
    assert emitted == "8=FIX.4.4|35=D|52=20260102-10:15:30|38=100|55=AAPL|54=1|59=0|10=000|"
    # Emission is idempotent: reading those bytes back emits them again.
...
=== "JavaScript"
    ...
    // quantity it is about - then the content with the side as its wire code,
    // the day order the dictionary derived for it, and the trailer.
    assert.equal(emitted, '8=FIX.4.4|35=D|52=20260102-10:15:30|38=100|55=AAPL|54=1|59=0|10=000|')
    // Emission is idempotent: reading those bytes back emits them again.
...
- For streamed Arrow output, [`write_arrow_reader`](arrow.md#back-to-the-wire)
  rebuilds each row's semantic message from its projected columns and residual
  `fixentries`, then writes one line with the codec's separator.
