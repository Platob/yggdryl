...
| Market operations | `market_operations(&self)` reads this message as typed graph operations; `into_market_operations(self)` moves a direct message's held event; `TryFrom<FixMsg> for MarketOperation` requires exactly one result. Order and quote categories are direct, an `EXEC` or `TRAD` category is direct only when this observation is an actual execution, and `W` / `X` expand `NoMDEntries(268)` in source order. All three doors are Rust-only |
...
| Bindings | The message is Rust, Python and JavaScript; conversion into graph market operations is Rust-only |

## Market operations

`market_operations()` is the borrowed door and clones the one base `MarketEventData`; `into_market_operations()` is the consuming door and moves that holder without cloning for one direct operation. The projected generic event is finalized once at that boundary, so its identity derives only from the facts the operation schema carries and it can pass directly through the graph Arrow round trip; the FIX message keeps its richer FIX-content identity before conversion. A message whose `msgcat` is `ORDR` becomes `Order`, one whose category is `QUOT` becomes `Quote`, and an `EXEC` or `TRAD` becomes `Execution` only where `Event::is_execution()` says this observation itself is an execution. Another category, a non-executing execution/trade message, or a `BOOK` message other than `W` or `X` is an `InvalidRecord` at `MsgType(35)`. `TryFrom<FixMsg> for MarketOperation` uses the consuming door and refuses unless it yields exactly one operation.

For `W` and `X`, the converter derives `entries()` once, requires exactly one `NoMDEntries(268)` group, walks its occurrences once in source order, and returns one operation per occurrence. An empty `W` returns one scoped `MarketOperation::Snapshot` control so an authoritative empty book can clear stale depth; an empty `X` states no changes and returns none. Root context is inherited and an occurrence's stated value overrides it. Root `MDReqID(262)` is taken from its lifted message field because lifting deliberately removes it from the residual entry tree. Converter-owned occurrence identifiers are cleared before the current occurrence is written, so a missing `MDEntryID`, reference, coordinate or application sequence cannot leak from the message event while unrelated caller identifiers survive. The entry type decides the concrete operation:
