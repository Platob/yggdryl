...
| `MDEntryType(269)` | MarketOperation | Side |
| --- | --- | --- |
...
A message goes by the names the fields of its dictionary state, and the three [`IdMap`](../graph.md#contract)s `Operation` answers - `get_accountids()`, `get_userids()` and `get_altids()` - are rebuilt from those fields at every settle, each entry under the upper-cased name of the field that stated it: a view of the row, never a store of its own. The source table lives in `rust/src/fix/msg.rs` until the dictionary carries it as a `FIX:idmap` property; the party roles are read from the `Parties` group beside it.
