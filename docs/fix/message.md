...
| Graph | `FixMsg` implements `Element`, `Event`, `Market` and `MarketOperation` - so `MarketEvent` and `MarketOperationEvent` - through its event: `is_after` is the instant, `finalize` settles the identity again, which re-derives every market fact from the FIX fields the message states unless a caller or a walk wrote one through the traits. `with_previous` first compares the complete `msgsesseventid`; equality forces a full content-and-graph merge instead of following, with the latest recorded observation as reference and the earliest `recdunix` and `execunix` retained. Otherwise it is `MarketEvent::following_market`, which records the predecessor's `curruuid` and `currunix` as `prevuuid` and `prevunix`, puts the message one place after it, `seqnum`, and carries the market facts the chain is about - not the operation's: the time in force, the accounts, the users and the followed alternate identifiers stay this message's own. `merge_with` is `MarketEvent::merging_market_event` under the same recording-clock rule, and `restating` the operation restatement, `graph::market::restating_operation`; import the traits to call them |
| Market operations | `market_operations(&self) -> Result<Vec<BookInput>>` reads this message as the [inputs a book takes](../graph.md#contract) - an `Operation`, a `Trade` or a snapshot `BookControl`; `into_market_operations(self)` moves a direct message's held root event; `FixMarketIterator` streams sorted messages without allocating a vector for direct messages; `FixCodec::book_arrow_reader` composes that iterator with `BookIterator` and bounded Arrow output; `TryFrom<FixMsg> for BookInput` requires exactly one result. Order and quote categories are direct, an actual `EXEC` is one execution, and only an initial TradeCaptureReport `35=AE` whose `TradeReportTransType(487)` is absent/New and whose `ExecType(150)` is absent or execution-like becomes one composite trade built from `NoSides(552)`; non-New/cancel/correct/reverse/status AE are refused. The composed book reader skips administration, requests, acknowledgements and non-executing reports; standalone conversion remains strict. `W` / `X` expand `NoMDEntries(268)` in nondecreasing effective time, retaining source order for ties. The operation conversion is Rust-owned and the composed Arrow reader is exposed by Python and JavaScript |
...
`market_operations()` is the borrowed door and clones the one base `MarketOperationEventData`; `into_market_operations()` is the consuming door and moves that root holder without cloning for one direct or composite operation. The projected generic event is finalized at that boundary, so its identity derives only from the facts the operation schema carries and it can pass directly through the graph Arrow round trip; the FIX message keeps its richer FIX-content identity before conversion. Its `marketoperationid` is the `int32` code read from `msgcatcodeset`: `10` (`ORDR`) becomes an `Operation` of kind `Order`, `14` (`QUOT`) one of kind `Quote`, and `8` (`EXEC`) one of kind `Execution` only where `Event::is_execution()` says this observation itself is an execution. Category `21` (`TRAD`) becomes a composite `Trade` only for an initial TradeCaptureReport `35=AE`: `TradeReportTransType(487)` must be absent or New (`0`, with legacy `N` or `New`), and `ExecType(150)` must be absent or execution-like (`F`, with legacy `1` or `2`). A non-New or cancel/correct/reverse/status AE, every `AD`, `AQ` and `AR`, another category, a non-executing execution message, or a `BOOK` message other than `W` or `X` is an `InvalidRecord` at `MsgType(35)`. `TryFrom<FixMsg> for BookInput` uses the consuming door and refuses unless it yields exactly one input.
...
A message goes by the names the fields of its dictionary state, and the three [`IdMap`](../graph.md#contract)s `MarketOperation` answers - `get_accountids()`, `get_userids()` and `get_altids()` - are rebuilt from those fields at every settle, each entry under the upper-cased name of the field that stated it: a view of the row, never a store of its own. The source table lives in `rust/src/fix/msg.rs` until the dictionary carries it as a `FIX:idmap` property; the party roles are read from the `Parties` group beside it.

| Map | Key | Source |
| --- | --- | --- |
| `accountids` | `ACCOUNT` | `Account(1)` |
| `accountids` | `CUSTOMERACCOUNT` | the `PartyID(448)` of the first `Parties` occurrence whose `PartyRole(452)` is `24` |
| `userids` | `SENDERSUBID`, `ONBEHALFOFSUBID` | `SenderSubID(50)`, `OnBehalfOfSubID(116)` |
| `userids` | `ENTERINGTRADER`, `EXECUTINGTRADER` | the `PartyID(448)` of the first `Parties` occurrence whose `PartyRole(452)` is `36` or `12` |
| `altids` | `ORDERID`, `SECONDARYORDERID`, `CLORDID`, `ORIGCLORDID`, `EXECID`, `TRDMATCHID`, `QUOTEID`, `QUOTEREQID`, `MDREQID`, `TRADEID` | `OrderID(37)`, `SecondaryOrderID(198)`, `ClOrdID(11)`, `OrigClOrdID(41)`, `ExecID(17)`, `TrdMatchID(880)`, `QuoteID(117)`, `QuoteReqID(131)`, `MDReqID(262)`, `TradeID(1003)` |

A value that states nothing - empty, `null`, `none`, `n/a`, `[n/a]` - adds nothing. A write goes through the field a key is read from: `insert_accountid`, `insert_userid` and `insert_altid` fill only an absent key, writing its field and answering `false` for a held one; `remove_*` nulls the field; `set_accountids`, `set_userids` and `set_altids` write every entry that moved and null every key the new map lacks; each settles the identity again, and the map is rebuilt from the row on the way out, so what a map answers is always what the row states. A key no top-level field states - a party-role key such as `CUSTOMERACCOUNT`, or a name the table lacks - is a located `InvalidRecord` at `$.altids.<KEY>` (`$.accountids.<KEY>`, `$.userids.<KEY>`) on every write that would have to land it or null it, because a map that is a view of the row cannot hold what the row cannot say; a plain holder, `MarketOperationEventData`, accepts any key. The [lifecycle](lifecycle.md#a-chain-is-named-by-its-cross-code) joins a chain by `altids`, and an operation that follows another carries its accounts, its users and the order's own alternate identifiers, `graph::FOLLOWED_ALTIDS`, never an execution's or a quote's. The maps are no column of the [fixed row](capture.md#the-crates-own-columns) - the fields that state them are - and the `identifiers` Map (65020) that once carried a message's names is retired, its tag never reused.
