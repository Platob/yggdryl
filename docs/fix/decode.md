...
- A value that will not type is null in the row and still exactly as it arrived in the entries, ready for emission; the refusal is silence rather than an error - except a stated clock the identity is settled against (`SendingTime(52)`, `TransactTime(60)`, `currunix`, `creaunix`), whose unreadable value is a located error item.
