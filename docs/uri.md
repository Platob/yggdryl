...
option. The scheme is a [`Scheme`](types.md) and the path is a `UriPath`, both of which validate
...
so in order. [`MediaType`](types.md) splits that into a base and its transparent encodings, which
...
canonical `file:` URL is also what [`local`](holder.md) stores as the entire state of a handle.
...
`MimeType::FILE`, and a remote URL falls back to the [`MimeType`](types.md) its name already
implies.
...
names replaced; [holder.md](holder.md) does exactly that.
