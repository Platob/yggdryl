...
=== "Rust"
    ...
    let value = yggdryl::text::from_str_with(r#"{"port": "{{ PORT }}"}"#, Format::Json, &loading)?;

    // A scalar that is exactly one placeholder adopts the resolved type.
...
=== "Python"
    ...
    value = json.loads('{"port": "{{ PORT }}"}', placeholders={"PORT": 8080})
