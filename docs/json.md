...
JSON deliberately has no `{{ }}` placeholder support: it is a data interchange
format, and a configuration document that wants templating is better written
as [YAML](yaml.md) or [TOML](toml.md), where the
[substitution](text.md#jinja-style-placeholders) lives. Passing placeholders
to a JSON load is refused by name rather than silently reading `{{ NAME }}`
as literal text.
