...
`text/plain` uses the same record methods as IPC, Parquet, and Avro. Physical
lines are records by default; `TextOptions` can instead frame logical records
from `rowheader` matches, with a bounded body prefix and an explicit leading-
fragment policy. Setting `with_rownum` / `withRownum` records the first physical
line of each result. The complete pre-read schema and byte-level contract live
in [plain-text records](media.md#plain-text-records).
...
methods. [Plain-text records](media.md#plain-text-records) defines the schema,
parsing order, errors, examples, and benchmark commands.
...
`yggdryl::holder::fs::FileSystem` is the one Arrow-compatible storage seam. It owns
filesystem metadata, directory and file lifecycle operations, native copy/move, and stateful byte
streams. `IOBase` is the one public bound resource handle above that seam. Yggdryl supplies
storage and text/media behavior; downstream libraries continue to own Iceberg catalogs, reads,
commits, caches, and their `FileIO` adapters.

### Bind an existing filesystem first

Pass the filesystem and its opaque path separately. `from_fs` never parses, decodes, or
normalizes `path`, including when it contains `://`.
...
The filesystem receives the literal object name `bucket/v=a%2Fb.bin`. The percent escape is
not decoded into a slash. The same rule preserves `%25`, `+`, repeated slashes,
non-ASCII text, and a literal `://` in an injected path.
...
`MemoryFileSystem` and `LocalFileSystem` are complete reference implementations.
Custom Rust implementations implement the same public object-safe `FileSystem` trait.
...
Yggdryl parses the URI with its own `Uri`/`Url` implementation. The parsed
authority supplies credentials, endpoint, bucket, region, and addressing configuration. One raw
path slice is retained so object-key escape spelling is never reconstructed or decoded.
...
The `options` mapping overrides URI query configuration. Supported S3 settings are access key,
secret key, session token, endpoint override, `http`/`https` transport, region,
anonymous mode, and automatic/path/virtual addressing. Python forwards them to
`pyarrow.fs.S3FileSystem`. Arrow JS has no S3 filesystem, so JavaScript validates and resolves
the URI but reports `Unsupported`; bind a caller-owned S3 handler with `fromFs`.

### Bound facts and identity

Every parent, child, listing result, and glob result retains:
...
`uri` is intentionally explicit because it can contain secrets. Use `masked_uri` in Python
or `maskedUri` in JavaScript for errors, logs, and snapshots. Repr/debug output and filesystem
errors never reveal user information, secret keys, or session tokens.

`same_location`/`sameLocation` requires both filesystem equality and byte-for-byte path
equality. A filesystem type name and path are not identity: two subtree, endpoint, credential
scope, or custom-handler instances may expose different bytes at the same path.
...
Each open retains one backend stream. Reads and writes do not repeat metadata lookup or reopen the
file for each chunk, and output is not accumulated into one whole-object buffer. Always close the
stream or use Python context management. Close is idempotent, flushes at most once, and retains the
first write/flush/close failure so a repeated close cannot hide it.

Python returns the original `pyarrow.NativeFile`. It forwards `compression`,
`buffer_size`, and output `metadata` exactly. Native yggdryl streams exposed through
`IOCursor` satisfy Python binary-file behavior and can be passed to `pyarrow.PythonFile`
without materializing the resource.

### Errors and metadata

`info()` returns `pyarrow.fs.FileInfo` in Python. JavaScript `ArrowFileInfo` uses
`bigint` for byte size and UTC modification time in nanoseconds. Rust `FileInfo` keeps size
and modification time optional, so a zero-byte file, an unknown mtime, and an absent path remain
different states.

Only metadata lookup represents absence as Arrow `NotFound`. Strict opens and mutations preserve
not-found, permission, already-exists, not-a-directory, is-a-directory, directory-not-empty,
unsupported, and transport failures. High-level reads that intentionally treat absence as empty
handle only typed not-found; they never suppress permission or transport errors.

Listings are ascending by exact path. Native recursive walks load at most one directory before
yielding; foreign eager results are sorted once. A listing yields its first error and then remains
exhausted. A missing selector base raises unless `allow_not_found`/`allowNotFound` is true.
Globs use the longest fixed prefix and preserve all bound-location facts.
...
Equal filesystems receive exactly one native `copy_file` or `move` call and no
client-side byte stream. A cross-filesystem copy uses one input and one output stream with bounded
chunks, publishes the target only after complete success, and removes its temporary output after a
failure. A missing or failed source therefore neither truncates an existing target nor creates a
missing one. Cross-filesystem move copies completely before deleting the source.
...
Root-content deletion is unreachable through an empty or broad ordinary delete. The explicit
root method accepts only a handle whose raw bound path is exactly empty. Backends that cannot
provide root deletion, append, or another optional capability return typed `Unsupported`; yggdryl
does not emulate different semantics.

### JavaScript handler protocol

Arrow JS supplies no filesystem backend. The package therefore exports the synchronous
`FileSystemHandler`, `FileSelector`, `ArrowFileInfo`, `RandomAccessReader`,
`ByteReader`, `ByteWriter`, `OutputMetadata`, and typed filesystem-error protocols.
...
Handler calls stay synchronous and on the JavaScript isolate that supplied the handler. Stream
objects have explicit `close()` and optional `Symbol.dispose` behavior. Sizes, offsets, and
nanosecond mtimes use `bigint`.

### Performance gates

Normal conformance tests enforce one retained stream per transfer, bounded retained payload,
zero stream operations for same-filesystem copy/move, and one-directory-at-a-time native recursive
listing. The filesystem benchmark uses at least 64 MiB, identical chunk sizes, warm-up, and median
throughput against direct PyArrow/local or native local operations; it fails if the wrapper is more
than 25% slower. Timing is not used as evidence for streaming correctness.
