...
;(() => {
  ...
  const manifest = () => {
    if (pending === null) {
      ...
      pending = fetch(address).then((answer) => {
        ...
        data,
        // The vocabularies, by the name a field's `FIX:codeset` states: the
  ...
  function fieldDetail(field, category, navigate, sets) {
    ...
    if (meta['FIX:codeset']) {
      // A vocabulary is the dictionary's, not the field's: `FIX:codeset` states
  ...
  function renderCatalog(root, held) {
    ...
    const show = () => {
      ...
      const matches = held.definitions.filter((entry) =>
        ...
        (coded.value !== 'codes' || metadata(entry.field)['FIX:codeset'] !== undefined) &&
