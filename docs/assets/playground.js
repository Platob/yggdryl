...
;(() => {
  ...
  /** A plain table with a header row: the shape a decoded frame is shown in. */
  const grid = (headings, rows, className) => {
  ...
  /** A section with a heading, shown only where it has something to say. */
  const section = (title, node) => {
    const held = make('div', 'ygg-pg__section')
    held.append(make('h4', 'ygg-pg__heading', title), node)
    return held
  }

  /**
   * One captured line at a time: what the package made of it, what arrived,
   * and what it derived. Every value comes from the manifest, so this is the
   * real reader's answer and not a second implementation of one.
   */
  const renderFix = (root, data) => {
    const group = data.fix
    ...
    const show = (index) => {
        ...
        view.append(
          section(
            ...
            grid(
              ...
              frame.columns.map((column) => [column.column, column.name, column.dtype, column.value]),
              'ygg-pg__widths',
            ),
          ),
        )
      }
      if (frame.arrivals.length > 0) {
        view.append(
          section(
            'What arrived',
            grid(
              ['tag', 'key', 'value'],
              frame.arrivals.map(([tag, key, value]) => [tag === '0' ? '-' : tag, key, value]),
              'ygg-pg__widths',
            ),
          ),
        )
      ...
      const derived = [
        ['symbolticker', frame.ticker],
        ['timestamp', frame.clock],
        ['unixpartition', frame.partition],
      ].filter(([, value]) => value !== null)
      if (derived.length > 0 || frame.lift.length > 0) {
        view.append(
          section(
            'What it derived',
            detail([...derived, ...frame.lift].map(([name, value]) => [name, value])),
          ),
        )
      }
      if (frame.unmapped.length > 0) {
        view.append(
          section(
            'What no dictionary explained',
            detail(frame.unmapped.map((key) => ['unmapped', key])),
          ),
        )
      }
      if (frame.anomalies.length > 0) {
        view.append(
          section(
            ...
            detail(frame.anomalies.map((held, at) => [`anomaly ${at + 1}`, held, false])),
          ),
        )
      }
      view.append(call(frame.call))
    }

    const built = list(
      frames,
      (frame) => frame.line.length > 56 ? `${frame.line.slice(0, 55)}…` : frame.line,
      (index) => {
        built.press(index)
        show(index)
      },
    )
    root.append(built.items, view)
    built.press(0)
    show(0)
  }

  /**
   * Every column of the fixed row, filtered as you type.
   *
   * Eighty-nine columns is more than anyone scrolls, and the question a reader
   * actually has is "which column holds X" - so the filter matches the tag,
   * the field name and the wording alike.
   */
  const renderFixSchema = (root, data) => {
    const group = data.fix
    const holder = make('div', 'ygg-pg__controls')
    const label = make('label', null, 'Filter')
    label.setAttribute('for', 'ygg-fix-schema')
    const box = make('input', 'ygg-pg__input')
    box.id = 'ygg-fix-schema'
    box.type = 'search'
    box.placeholder = 'symbol, 55, price…'
    holder.append(label, box)

    const counter = make('p', 'ygg-pg__counter')
    counter.setAttribute('aria-live', 'polite')
    const view = make('div', 'ygg-pg__view')

    const show = () => {
      const wanted = box.value.trim().toLowerCase()
      const kept = group.columns.filter(
        (column) =>
          wanted === '' ||
          column.column.includes(wanted) ||
          column.name.toLowerCase().includes(wanted) ||
          column.description.toLowerCase().includes(wanted),
      )
      counter.textContent =
        `${kept.length} of ${group.columns.length} columns` +
        `, from a dictionary of ${group.size} fields`
      view.textContent = ''
      view.append(
        grid(
    ...
    root.append(holder, counter, view, call(group.call))
    show()
  }
    ...
    manifest().then(
      (data) => {
        ...
        for (const root of roots) {
          ...
          else if (role === 'fix') renderFix(root, data)
          else if (role === 'fixSchema') renderFixSchema(root, data)
