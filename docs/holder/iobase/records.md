...
- `start_rownum` / `startRownum` -> records the first physical line of each result; its nonnegative row numbers supply the text line's default event `seqnum`, while without it the zero-based physical index does.
