...
| | |
| --- | --- |
| Owns | `yggdryl::s3::{S3Path, S3Folder, S3File}`, the `IOPath`, `IOFolder`, `IOFile` roles of [Holder](../index.md) |
...
An [Iceberg table](../../media/index.md#iceberg) over a store touches only the
...
accounting suite (`accounting::iceberg` in `rust/tests/s3/mod_.rs`) holds exactly.
...
[Iceberg writes](../../media/index.md#iceberg).
