...
| a list/array with any fractional value | `float64` serie |
...
The `int64` / `float64` split follows each language's idiom: Python distinguishes
`int` from `float` (`42` → `int64`, `1.5` → `float64`, `42.0` → `float64`), while
