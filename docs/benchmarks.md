...
| FIX | [Arrow](fix/arrow.md) | `fix/pipeline` over `rust/tests/fix/ulbridge.log`, a bridge's own log repeated to eleven megabytes: the text reader alone, `parse_text_arrow_reader` whole, the codec over the framed bodies, and each pass over a built message - its row, its batch, `enrich_messages`, `lifecycle`, `digest`; one Linux x86_64 container release run |
| FIX | [Capture](fix/capture.md) | `fix/plugin`: a bulk configuration document walked into its configurations and read into messages, at three sizes; one Linux x86_64 container release run |
