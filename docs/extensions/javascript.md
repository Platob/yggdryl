...
call the native constructor. Variadic `Uri.joinPath` replaces the `/` operator
idiom.
...
is validated and memoized per class.
...
factories take `(count, unit, timezone)`.
...
and `asBytes`/`asUtf8` borrow content.
...
`Expression` exposes the same names as lazy tree builders.
...
`at`, `get`, `has`, and `path` return exact native children, while `set` and
`remove` rebuild without mutating the source.
...
Typed identifiers and typed HTTP values (`contentType`, `etag`) are validated
accessors, not map keys. Each well-known protocol is a getter over a live `Map`
...
`field.protocol(name)` takes a runtime scheme, and a schema says which columns
a path spells out.
...
`append_bytes`, camelCased. Over each sits one inferring entry point.
...
`BatchReader` is the primitive record shape, and `BatchReader.from` accepts
another reader, an Arrow JS `Table` or `RecordBatch`, an array of batches, or
IPC bytes.
...
Each batch crosses as its own IPC stream, so its schema travels
with it. `intoIpc` and `intoTable` drain the reader at once.
...
`mode` is `'overwrite'`, `'append'`, or `'merge'`; table and batch methods copy
one IPC stream.
...
Plain objects infer one struct field through Arrow JS; a class instance uses its
static `intoStructField` getter. Records are pulled in chunks of
...
`Digest`. XXH32 answers a `number`, the wider algorithms `bigint`.
...
`fix.installGlobalRegistry()`, `fix.STANDARD_BRANCH`, and
`fix.STANDARD_TAG_LIMIT` are the whole surface. The `fix:` vocabulary is six
accessor pairs on the `field.fix` view.
...
| `field.fix.id` | `'branch:tag'`, `null` exactly when `fix:tag` is absent |
| `fromHandle`, `writeInto` | an `IOBase`, a `Url`, or the string naming one |
| iteration | registry by canonical identifier, message in declared order |
...
- `maxRowSize = 0` or `maxByteSize = 0` -> append no-ops, overwrite publishes
  the declared empty `options.field`, merge is rejected.
