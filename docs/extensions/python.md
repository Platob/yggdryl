...
| crate inventory | `fix_crate_fields()` answers 27 definitions: 26 scalar fields on tags 65001-65019 and 65021-65027, plus the `altids` Map group at 65020; `FixRegistry()` also seeds `sendingtime` (52) and `transacttime` (60), so it holds 28 scalar fields; `len(registry)` and registry iteration count scalar fields only, while `definitions("groups")` includes `altids` |
