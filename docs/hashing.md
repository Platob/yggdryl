...
| FIX identities | a message's `hashcode` is the XXH3-64 of what the message *states* - the event's facts, the text, the metadata, the header fields it stated, then the entry tree, and never the columns a row happened to lay them out in, so a message read back out of a row is the same message - and its `crosshashcode` the XXH3-64 of the code its chain shares; `curruuid` is the UUIDv7 [`TxHash`](#txhash-values) couples its instant and `hashcode` into, and `crossuuid` the UUIDv8 of `crosshashcode`. Never a second engine, and the recipes live with [FIX messages](fix/message.md#typed-tags) and the [graph](graph.md) traits that derive them |
