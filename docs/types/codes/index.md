...
The fourteen registered codes: an identity over a published registry, the width its standard fixes, and the Arrow extension name that identity rides.
...
| Aspect | Rule |
| --- | --- |
| Owns | Fourteen `DataType` variants, fourteen `Field` leaves, fourteen `Scalar` variants, and the `CodeValue` contract their leaf values answer; the family is the code range of `DataTypeId` bytes, not a type |
...
| Value rank | The fourteen share one value rank, so what separates two codes of the same bytes is the identity their datatypes sort by: `Side("BUY")` and `TimeInForce("BUY")` are two values |
| Rust only | `DataType::CODES`, the fourteen leaf value types, `CodeValue` and its `merge_with`, `Scalar::code_storage` and `Scalar::is_code` |

The contract every registered code answers lives in `rust/src/code.rs`: the `CodeValue` trait - `WIDTH`, `as_str`, `storage`, `merge_with` - and the two crate-internal builders `code_leaf!` and `code_value!` that a code file declares its value with. Each of the fourteen is then one file of its own, holding its datatype, its field marker and its value in that order.
...
=== "Python"
    ...
    assert [(DataType(name).id, DataType(name).code_width) for name in
            ("country", "ccy", "mic", "cfi", "isin", "cusip", "sedol",
             "side", "state", "timeinforce", "bbg", "figi", "unit", "ric")] == [
...
=== "JavaScript"
    ...
    assert.deepEqual(
      ['country', 'ccy', 'mic', 'cfi', 'isin', 'cusip', 'sedol', 'side', 'state', 'timeinforce', 'bbg', 'figi', 'unit', 'ric']
        .map((name) => new DataType(name).codeWidth),
      [2, 3, 4, 6, 12, 9, 7, 8, 10, 8, 32, 12, 32, 32],
    )
...
express the same column as the datatype alone. Four registered codes ship
declared - `CCY`, `COUNTRY`, `MIC` and `CFI`, each over the base a caller
subclasses for a vocabulary of its own: `Ccy`, `Country`, `Mic` and `Cfi`;
`fixed_ascii(width)` builds one over any fixed width.
...
- `from_logical_name` -> the shipped `COUNTRIES`, `CURRENCIES`, `MICS`, `SIDES`, `DIRECTIONS`, `STATES`, `TIMESINFORCE` listings, `prebuilt()` in either binding; `"Exchange"` -> `MICS`; a registered name with no listing, such as `tenor` or any of the six securities identifiers, -> an empty enum.
...
=== "Rust"
    ...
    ```bash
    cargo test --features "parquet iceberg" --manifest-path rust/Cargo.toml -p yggdryl --test root -- ascii::fields ascii::leaves bbg::value bbg::datatype cfi::coded code::datatypes code::securities cusip::securities figi::securities ric::value:: ric::datatype:: sedol::securities state::coded string::enumerated string::listings timeinforce::coded
