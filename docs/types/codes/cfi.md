...
| Aspect | Rule |
| --- | --- |
| Owns | `cfi`, `CfiCodeType`/`CfiCodeField`, the `CfiCode` value and `Scalar::CfiCode`, and the `CFI_CATEGORIES` grid with `CfiCategory`/`CfiGroup` |
...
=== "Rust"
    ...
    assert_eq!(DataType::from_str("cfi")?, DataType::CfiCode);
    assert_eq!(DataType::CfiCode.to_string(), "cfi");
    assert_eq!(DataType::CfiCode.kind(), DataTypeKind::Code);
    assert_eq!(DataType::CfiCode.code_name(), Some("cfi"));
    assert_eq!(DataType::CfiCode.code_width(), Some(6));
    assert_eq!(DataType::CfiCode.fixed_byte_width(), None);
    // Six bytes with an identity is not six bytes of text.
    assert_ne!(DataType::CfiCode, DataType::fixed_ascii(6)?);
    ```
...
`CfiCodeField` is the typed marker; Python and JavaScript name the factory `cfi`.

=== "Rust"

    ```rust
    use yggdryl::{CfiCodeField, DataType, Field};

    let classification = CfiCodeField::unit("classification", true);
    assert_eq!(classification.dtype(), &DataType::CfiCode);
    assert_eq!(
        classification.to_field(),
        Field::new("classification", DataType::CfiCode, true)
    );
    ```
...
=== "Rust"
    ...
    ```rust
    use yggdryl::{CfiCode, DataType, Scalar};

    let share = DataType::cfi().scalar("ESVUFR")?;
    assert_eq!(share, Scalar::CfiCode(CfiCode::new("ESVUFR")?));
    ...
    let refused = CfiCode::new("ESVUFRX").unwrap_err().to_string();
...
=== "Rust"
    ...
    let classification = Field::new("classification", DataType::CfiCode, false);
...
`CfiCode::is_classified` answers the grid, `category_of` and `CfiCategory::group` walk it, and `CfiCode::coarse` builds a code from a category and a group a caller inferred with every attribute unknown - falling back to the category's own Others group rather than to `X`, because `X` is not a group. Rust only.

```rust
use yggdryl::CfiCode;

assert_eq!(CfiCode::LENGTH, 6);
assert_eq!(CfiCode::UNKNOWN, 'X');
...
let equities = CfiCode::category_of('E').expect("a category");
...
- `CfiCode::merged` answers `None` for anything that is not two well-formed codes; `merge_with` turns that `None` back into this code.
- Python declares the vocabulary over the width as `yggdryl.enums.CFI`; `StringEnum::from_logical_name("cfi")` answers an enum of no members, because the grid is a rule rather than a listing.
...
- A lifecycle may learn a missing CFI attribute only under an already-valid [ISIN](isin.md) in its own [graph walk](../../graph.md).
...
=== "Rust"
    ...
    ```bash
    cargo test --features "parquet iceberg" --manifest-path rust/Cargo.toml -p yggdryl --test root -- cfi_code::coded code::datatypes state::coded timeinforce::coded
    ```
