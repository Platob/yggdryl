...
The listing is a vocabulary rather than a gate: a value it does not carry - a venue's `GTX`, a `D` no shipped version names - is held as stated. `TimeInForce::from_spelling` reads a wire code unfolded, else a shipped name folded, else the value as stated, and stores the code; `TimeInForce::read` is the same reading as a refusal, naming `TimeInForce(59)`, for a value past eight bytes. Rust only.
