...
`Unit::none()` is the empty unit, the value stated as none: it is the code's default, what [`Market::get_unit`](../../graph.md#contract) answers where a market states none and a `Lane` leaves out, and a [merge](index.md#the-code-family-value) takes the other unit over it.
