...
`is_bid` and `is_ask` say which lane a side takes, which is what a FIX lift and a market element fill a lane by. `BUY` and `BUYMINUS` take the bid; `SELL`, `SELLPLUS`, `SSHORT`, `SSHORTEX` and `SELLUND` take the ask. Everything else takes neither: a cross is both sides at once, `OPPOSITE` means "whatever the other leg was", and `ASDEF`, `UNDISC` and a side stated as none say nothing about a lane. Domain knowledge written where a reviewer can check it, because Orchestra does not publish it. Rust only.
