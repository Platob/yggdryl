...
| `set_value` on a variable-length leaf | one rewrite of the offsets and the payload, every time: a run is as long as it is, so every later offset moves |
| `push_value` and `set_value` on a viewed, fixed-width or boolean leaf | one rewrite: a view word names which buffer holds it, a fixed payload has no builder to hand back, and a bitmap has none either. That is what those layouts cost a write; their reads stay one load |
...
- Six layouts the crate's own datatypes project to have no column here yet, and are refused by name rather than read as something else: dictionary, run-end and union, and three of the five sequence leaves - `list_view`, `large_list_view` and `fixed_size_list`. `list` and `large_list` are the two a sequence column holds, and a mapping rides the second marker on the first of them.
...
- `Serie::field` answers `None` for a run, because a run declares none. `Serie::require_field` is the same read as a refusal, and it is what a record's child goes through on `with_child`, and what a batch or a reader root goes through — a run cannot stand where Arrow names a layout.
- `items_mut`, `child_mut` and the `as_*_mut` chain hand out `&mut Serie`, which is what lets a leaf's buffers be written in place. It is also what lets a run, or a serie of the wrong length for the cut, be put where a column belongs. Arrow names a layout there and neither can stand in it, so laying such a column out answers an empty array rather than naming what was wrong: the layout door is infallible today, and only `require_field` and `with_child` refuse. Replace a child through `with_child` rather than by assigning through the mutable accessor.
