...
| Fixed payloads | a number is its little-endian bytes and nothing else, because the identifier says how wide it is: `int32` is two bytes and four, a `uuid` two and sixteen, a boolean two and one, a null two and none; a decimal is its scale as one byte, then its coefficient, and a `decimal` or `bigdecimal` its units alone, the scale being the datatype's; a clock is its unit as one byte, its zone as a length-prefixed text where the leaf has one, then its count |
