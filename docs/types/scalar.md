...
| containers | `Sequence`, `Mapping`, `Record`, `Variant`; a `Sequence` holds a [`Serie`](serie.md) - a schema-free `Run`, or a column of one field - and `kind()` answers `sequence` for either, `as_sequence` lending a run's values and `sequence_rows` reading both |
