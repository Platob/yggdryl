...
An instance of such a class - or of any dataclass or named tuple - crosses as the record of its value fields. The class's field names are resolved on its first instance and remembered, so every instance costs one attribute read per field and nothing is re-inferred per value. `Class.into_field().scalar(instance)` is its canonical row, and the Python records writers land a stream of instances through that same value contract. A member holds a bare value, so a union member is spelled as the pair naming the branch [the union's own rule](nested/union.md#scalar) chooses - a `list[int] | str` member's list stays a list - and an enum member is the value it names. The writers read each instance onto the field the write declares by name, a column the class does not declare being null and a member the field does not declare left unread, or onto the class's own field when none is declared. Parquet has no union layout and refuses a union column by name. `read_records(Class)` and `json.loads(..., cls=Class)` read through a plan the class compiles once: a value that already is its annotation's exact class - `int`, `float`, `str`, `bool`, `bytes`, `Decimal`, `UUID`, `date`, one of them or `None`, or a union member's - reaches the constructor unchanged, a nested class's mapping is read by that class's own plan, and a list item by item. Every other value is cast losslessly or refused as before, naming the path it reached.
