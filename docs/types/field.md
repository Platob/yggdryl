...
=== "Rust"
    ...
    use yggdryl::{ArrowCastOptions, DataType, Nullability};
    ...
    let applied = root.apply_arrow_batch(&batch, true, true, true, ArrowCastOptions::new().with_nullability(Nullability::Default))?;
    ...
    assert_eq!(
        root.apply_arrow_batch(&applied, true, true, true, ArrowCastOptions::new().with_nullability(Nullability::Default))?,
        applied,
    );

    // The same shape, with no rows read and no batch pulled.
    let shape =
        root.apply_arrow_schema(batch.schema(), true, true, true, ArrowCastOptions::new().with_nullability(Nullability::Default))?;
    ...
    let mut stream = root.apply_arrow_reader(
        ...
        true,
        true,
        ArrowCastOptions::new().with_nullability(Nullability::Default),
    )?;
