...
The scan is planned by the filter that keeps the rows: a manifest-list summary answers first, then a manifest entry's partition tuple and column bounds. Pushdown and time travel are on [Reading](../media/iceberg/read.md).
