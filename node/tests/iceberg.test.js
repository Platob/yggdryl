...
test('numbering a schema is a copy, and the numbers are Arrow field ids', () => {
  ...
  assert.equal(numbered.dtype.getFieldAt(0).parquetFieldId, 1)
  assert.equal(numbered.dtype.getFieldAt(1).parquetFieldId, 2)
  assert.equal(numbered.dtype.getFieldAt(0).get('PARQUET:field_id'), '1')

  // Numbering starts where a caller says, so an evolution never reuses one.
  const later = iceberg.assignFieldIds(plain, 10)
  assert.equal(later.dtype.getFieldAt(0).parquetFieldId, 10)
})
...
test('creating a table numbers a plain schema itself, partitioning included', (t) => {
  ...
  assert.equal(table.schema.dtype.getFieldAt(1).parquetFieldId, 2)
...
test('a schema evolves through one recorded chain, committed once', (t) => {
  ...
  assert.equal(evolved.dtype.getFieldAt(0).dtype.id, 'int64')
  // The renamed column keeps its identifier; the added one is numbered fresh.
  assert.equal(evolved.dtype.getFieldAt(1).parquetFieldId, 2)
  assert.equal(evolved.dtype.getFieldAt(2).parquetFieldId, 3)
...
test('a schema is a document in both directions', () => {
  ...
  assert.equal(imported.dtype.getFieldAt(0).nullable, false)
})
