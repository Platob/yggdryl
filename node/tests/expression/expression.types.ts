...
const constant: Term = Term.literal(Scalar.fromJs('EUR'))
const typed: Term = Term.typedLiteral('int32', Scalar.fromJs(5))
...
const keyed: Term = named.key(Scalar.fromJs('k'))
const sumTerm: Term = Term.column('price').add('1')
const inferredSumTerm: Term = Term.column('price').add(1)
const nativeSumTerm: Term = Term.column('price').add(Scalar.fromJs(1))
...
const boundSelectorRow: Scalar = boundSelector.applyRow(Scalar.fromJs(['EUR', 1]))
