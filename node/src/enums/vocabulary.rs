...
pub fn enum_values_native() -> HashMap<String, Vec<String>> {
    ...
    HashMap::from([
        ...
        (
            "charsets".to_owned(),
            spell(&Charset::ALL.map(Charset::as_str)),
        ),
        (
            "digestAlgorithms".to_owned(),
            spell(&DigestAlgorithm::ALL.map(DigestAlgorithm::as_str)),
        ),
        (
            "ioKinds".to_owned(),
        ...
        (
            "pythonKinds".to_owned(),
            spell(&PythonKind::ALL.map(PythonKind::as_str)),
        ),
        (
        ...
        (
            ...
            spell(&EventColumn::ALL.map(EventColumn::name)),
        ),
        (
            "marketColumns".to_owned(),
            spell(&MarketColumn::ALL.map(MarketColumn::name)),
        ),
        (
            "operationColumns".to_owned(),
            spell(&OperationColumn::ALL.map(OperationColumn::name)),
        ),
    ])
}
