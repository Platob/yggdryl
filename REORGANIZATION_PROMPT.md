...
`ascii/` are separate families because the ASCII datatypes carry a value rule,
an Arrow extension identity, and — for `FixedAscii` and the four codes — a
width and a packed integer that UTF-8 does not; see the *Symmetry law* in
`SCALAR_HIERARCHY_PROMPT.md`, which requires each of these ten folders to own a
matching datatype enum, marker set, and value enum.
