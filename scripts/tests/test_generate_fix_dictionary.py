...
class FixCatalogGeneration(unittest.TestCase):
    ...
    def test_two_fields_reading_one_code_set_name_it_and_hold_it_once(self) -> None:
        ...
        named = [field["metadata"]["FIX:codeset"] for field in catalog["fields"]]
        self.assertEqual(["sourcecodeset", "sourcecodeset"], named)
        self.assertEqual({"sourcecodeset": [{"value": "1", "name": "CUSIP"}]}, code_sets)

    def test_party_id_source_family_keeps_the_bridge_proprietary_alias(self) -> None:
        catalog, code_sets = built(b'''<repository xmlns="http://fixprotocol.io/2020/orchestra/repository" version="FIX.5.0SP2">
          <codeSets>
            <codeSet name="PartyIdSourceCodeSet" type="String"><code name="Proprietary" value="D"/></codeSet>
            <codeSet name="OtherSourceCodeSet" type="String"><code name="Proprietary" value="D"/></codeSet>
          </codeSets>
          <fields>
            <field id="447" name="PartyIDSource" type="PartyIdSourceCodeSet"/>
            <field id="525" name="NestedPartyIDSource" type="PartyIdSourceCodeSet"/>
            <field id="901" name="PartyDetailIDSource" type="PartyIdSourceCodeSet"/>
            <field id="900" name="OtherSource" type="OtherSourceCodeSet"/>
          </fields>
        </repository>''')
        fields = {int(field["metadata"]["FIX:tag"]): field for field in catalog["fields"]}
        for tag in [447, 525, 901]:
            with self.subTest(tag=tag):
                self.assertEqual("partyidsourcecodeset", fields[tag]["metadata"]["FIX:codeset"])
        code = code_sets["partyidsourcecodeset"][0]
        self.assertEqual({"value": "D", "name": "Proprietary", "aliases": ["proprietary/customcode"]}, code)
    ...
    def test_message_type_is_text_reading_by_its_code_set(self) -> None:
        ...
        self.assertEqual("msgtypecodeset", field["metadata"]["FIX:codeset"])
