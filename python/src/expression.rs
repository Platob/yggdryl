...
use yggdryl::expression::{Bound as CoreBound, Statement as CoreStatement};
...
#[pyclass(name = "Expression", module = "yggdryl._native", frozen)]
...
impl PyExpression {
    ...
    fn bind(
        ...
        let supplied: Vec<(String, Value)> = match parameters {
            Some(parameters) => parameters
                .iter()
                .map(|(name, value)| {
                    Ok((name.extract::<String>()?, crate::value::from_py(&value)?))
                })
                .collect::<PyResult<Vec<_>>>()?,
            None => Vec::new(),
        };
        let borrowed: Vec<(&str, Value)> = supplied
            .iter()
    ...
    fn __richcmp__(&self, other: &Bound<'_, PyAny>, operation: CompareOp) -> bool {
        let Ok(other) = expression_from_value(other) else {
            return matches!(operation, CompareOp::Ne);
        };
        crate::compare(self.inner.cmp(&other), operation)
    }
}
...
#[pyclass(name = "Statement", module = "yggdryl._native", frozen)]
...
impl PyStatement {
    ...
    fn __richcmp__(&self, other: &Bound<'_, PyAny>, operation: CompareOp) -> PyResult<bool> {
        let Ok(other) = other.extract::<PyRef<'_, Self>>() else {
            return Ok(matches!(operation, CompareOp::Ne));
        };
        match operation {
            CompareOp::Eq => Ok(self.inner == other.inner),
            CompareOp::Ne => Ok(self.inner != other.inner),
            _ => Err(value_error("statements compare only for equality")),
        }
    }
}
