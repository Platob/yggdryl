"""Iceberg tables exchanged with Apache Spark, the format's reference
implementation.

An exchange rather than the mirror of one source file, so it is its own target
the way `rust/tests/interop/` is: the whole module is marked `spark_interop`
and skips at collection where no JVM, no pyspark or no iceberg-spark-runtime
jar is provisioned. That module-level skip is why it cannot sit beside the
ordinary Iceberg suite in `test_iceberg.py` - it would take those tests with
it wherever Spark is absent, and mark them `spark_interop` where it is not.

Provision it with `python scripts/setup_spark_interop.py`, then run
`python -m pytest python/tests/test_spark_interop.py -m spark_interop`.
"""
...
class TestSparkInteropPartitioning:
...
class TestSparkInteropCompaction:
