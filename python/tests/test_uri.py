...
def test_uri_components_path_collection_and_value_protocols() -> None:
    ...
    assert Uri.from_json(value.to_json()) == value
...
def test_windows_and_unc_paths_are_normalized_by_core() -> None:
    ...
    assert Url.from_path(path).to_uri() == drive
    ...
    assert drive.to_path() == "C:/Users/Ada Lovelace/report.parquet"
    assert drive.into_path() == drive.to_path()
    assert os.fspath(drive) == drive.to_path()
    file_url = Url.from_path(path)
    assert file_url.to_path() == drive.to_path()
    assert file_url.into_path() == drive.to_path()
    assert os.fspath(file_url) == drive.to_path()
    ...
    assert unicode_unc.to_path() == "//café/share/market data%.csv"
    assert Uri.from_path(unicode_unc.to_path()) == unicode_unc

    with pytest.raises(ValueError):
        Uri("https://example.com/data").to_path()
    with pytest.raises(ValueError):
        os.fspath(Uri("https://example.com/data"))
    assert Uri("file:///tmp/A%20B.csv").to_path() == "/tmp/A B.csv"
    assert Uri("file://server").to_path() == "//server/"
    with pytest.raises(ValueError):
        Uri("file:///tmp/a%2Fb.csv").to_path()
    with pytest.raises(ValueError):
        Uri("file:///tmp/a%5Cb.csv").to_path()
    with pytest.raises(ValueError):
        Uri("file:///tmp/data.csv?download=1").to_path()
    with pytest.raises(ValueError):
        Uri("file:///tmp/data.csv#row-1").to_path()
    with pytest.raises(ValueError):
        Uri("file:").to_path()
    with pytest.raises(ValueError):
        Uri("file:///tmp/%FF.csv").to_path()
    with pytest.raises(ValueError):
        Uri("file://user%40host/share/data.csv").to_path()
    with pytest.raises(ValueError):
        Uri("file:///%43%3A/data.csv").to_path()
    with pytest.raises(ValueError):
        Uri("file:///tmp/%C2%85/data.csv").to_path()
...
def test_url_converts_through_uri_without_binding_side_parsing() -> None:
    ...
    assert value.to_uri() == uri
    assert value.into_uri() == uri
    assert uri.to_url() == value
    ...
    assert Url.from_json(value.to_json()) == value
...
def test_urn_components_and_uri_conversion() -> None:
    ...
    assert Urn.from_json(value.to_json()) == value
