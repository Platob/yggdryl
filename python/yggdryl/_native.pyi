...
class RecordOptions:
    ...
    @property
    def row_element(self) -> str | None: ...
    @row_element.setter
    def row_element(self, row: str | None) -> None: ...
    @property
    def max_depth(self) -> int | None: ...
    @max_depth.setter
    def max_depth(self, max_depth: int) -> None: ...
    @property
    def max_input_bytes(self) -> int | None: ...
    @max_input_bytes.setter
    def max_input_bytes(self, max_input_bytes: int) -> None: ...
    @property
    def max_nodes(self) -> int | None: ...
    @max_nodes.setter
    def max_nodes(self, max_nodes: int) -> None: ...
    @property
...
def xml_dumps(value: object, name: str, *, indent: int = -2) -> bytes: ...
...
def xml_schema_dumps(field: object, *, indent: int = -2) -> bytes: ...
...
class FixFieldIterator(Iterator[Field]):
    ...
    the walk is tag-major and then by identifier.
    """
...
class FixDefinitionIterator(Iterator[Field]):
    __hash__: ClassVar[None]  # type: ignore[assignment]
    def __iter__(self) -> FixDefinitionIterator: ...
    def __next__(self) -> Field: ...

class MsgTypeIterator(Iterator[MsgType]):
    __hash__: ClassVar[None]  # type: ignore[assignment]
    def __iter__(self) -> MsgTypeIterator: ...
    def __next__(self) -> MsgType: ...

class MsgType:
    """One immutable message definition borrowed from its registry."""
    ...
    @staticmethod
    def _from_pickle(registry: str, index: int) -> MsgType: ...
...
class FixRegistry:
    ...
    name says ``id``, since a bare ``int`` elsewhere is a tag; a name and a
    dotted path are each a ``str``. The registry is one namespace: a bare tag
    or name uses the core's deterministic best match - the canonical holder
    before an alternate, the canonical name before an alias, under the one
    fold. Absence is a ``KeyError`` carrying the native message, a refusal a
    ...
    def get_definition(self, category: str, name: str) -> Field | None: ...
    def definition(self, category: str, name: str) -> Field: ...
    def definitions(self, category: str) -> FixDefinitionIterator: ...
    def add_definition(self, category: str, field: FieldLike) -> bool: ...
    def insert_definition(self, category: str, field: FieldLike) -> Field | None: ...
    def create_definition(self, category: str, field: FieldLike) -> None: ...
    def update_definition(self, category: str, field: FieldLike) -> Field: ...
    def remove_definition(self, category: str, name: str) -> Field | None: ...
    def get_msgtype(self, spelling: str) -> MsgType | None: ...
    def msgtype(self, spelling: str) -> MsgType: ...
    def msgtypes(self) -> MsgTypeIterator: ...
    def get_group_by_tag(self, tag: int) -> Field | None: ...
    def group_by_tag(self, tag: int) -> Field: ...
    def into_json(self) -> str: ...
    def stable_hash(self) -> int: ...
...
class FixCodec:
    ...
    ``parse_fixml_line`` a FIXML row, ``parse_plugin_line`` a bridge
    configuration document, and ``parse_pairs`` what a caller already split.
    Lines, records and documents answer a lazy ``FixMessages``; the FIX,
    Ullink, FIXML and pair readers answer one ``FixMsg``. Enrichment:
    ``enrich_message`` fills one message, ``enrich_messages`` a stream of them.
    ...
    def parse_plugin_line(self, body: bytes | bytearray | memoryview) -> FixMessages: ...
    ...
    def enrich_messages_arrow_reader(self, source: FixArrowSource) -> pyarrow.RecordBatchReader: ...
...
class Plugins(Iterator[Plugin]):
    __hash__: ClassVar[None]  # type: ignore[assignment]
    def __iter__(self) -> Plugins: ...
...
class Plugin:
    ...
    under, beside the attributes it stated. What the read asked and how the
    asking went is the transport's, and no part of the configuration.
    :class:`FixMsg` is the same facts typed against a dictionary, and the two
    cross both ways.

    A body that is not a Jolokia answer names no plugin, and naming none is
    what it answers: ``from_json_bytes`` and ``from_json_scalar`` iterate
    ...
    ``Plugin({"Name": "OrderRouting"})`` and the bytes spelling it are one
    ...
    @staticmethod
    def _from_pickle(attributes: str, mbean: str | None) -> Plugin: ...
    @staticmethod
    def from_json_bytes(body: bytes | bytearray | memoryview) -> Plugins: ...
    @staticmethod
    def from_json_scalar(document: object) -> Plugins: ...
    @staticmethod
    def from_fixmsg(message: FixMsg) -> Plugin: ...
    def into_fixmsg(self, codec: FixCodec) -> FixMsg: ...
    @property
    ...
    def __deepcopy__(self, memo: Any) -> Plugin: ...
    def __reduce__(self) -> tuple[Any, tuple[str, str | None]]: ...
    def __repr__(self) -> str: ...

def fix_generic_message(registry: FixRegistry | None = None, name: str = "fix") -> Field: ...
...
def fix_plugin_fields() -> list[Field]: ...
def fix_plugin_message() -> Field: ...
def fix_cfb_fields(
    location: IOBase | Url | str | PathLike[str],
    dialect: str | None = None,
) -> list[Field]: ...
