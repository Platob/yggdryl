...
- **P7-R8. `entries` is not the row restated** — the one place this brief
  ...
  (P7-R28) work. **Say exactly this in the doc comment**, or a reader will
...
- **P7-R15. The *source* version, when the caller names none** — the version
  ...
  1. **Tag 1128 `ApplVerID`** — the application version: `0`=FIX27,
     ...
     (P3-R1b). It wins because under FIXT.1.1 the session version says
...
- **P7-R16. Branch, when the caller names none.** The first step is identity,
  ...
  1. **The session names the dialect.** `SenderCompID(49)` and
     `TargetCompID(56)` are in every header, and P2-R9b bundles that pair
...
- **P7-R22.** `with_registry` finishes it, so existing validation and
...
- **P7-R27. Wire spellings belong to the FIX layer, never to the generic
...
- **P7-R37.** It rewrites into the key forms `from_pairs` takes —
...
11. A branch's declared default losing to a message's own `ApplVerID` and
    winning over `newest()` (P7-R15.3).
12. Tag-count inference picking the vendor dictionary, the tie rule, and an
    explicit branch suppressing inference (P7-R16.3).
...
25. Out-of-order and gapped indices (P7-R36); a counter of `2` against one
...
26. `entries()` holding every pair in arrival order with the untranslated
    spelling, beside a row holding the translated code (P7-R8); an
    unresolved key with `tag` `None` and `key` set (P7-R6).
