...
Ten phases across three prompt files. Each phase is complete work: it
...
| `03-messages.md` | 7, 9, 10 | the registry's explicit halves, `FixEntry`, `from_pairs`, the three text readers; lifting; Arrow batches in and out |

**Order.** `1 → 3 → 4 → 5`; `1 + 3 + 4 → 6`; `4 + 6 → 8`; `2 + 3 + 6 → 7`;
`7 + 8 → 9 → 10`. Phase 2 blocks on nobody.
