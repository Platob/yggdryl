...
Eleven phases across three prompt files. Each phase is complete work: it
...
| `03-messages.md` | 7, 9, 10, 11 | the registry's explicit halves, `FixEntry`, `from_pairs`, the three text readers; lifting; `from_record` and Arrow batches in and out; the crate's own `MsgHash` and `Direction` fields, and dedup |
