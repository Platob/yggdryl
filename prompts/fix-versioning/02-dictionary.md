...
- **P3-R2. "FIX Latest" is not a version and is never stored as one.** It
...
- **P3-R3. FIXT.1.1 is not modelled.** Session tags carry the application
  version that first defined them; the FIX page names FIXT as a known
  omission.
- **P3-R4. A pedigree is a `Version` and an optional extension pack.**
...
- **P3-R7. Field names are the FIX name, lower-cased.** `lastqty`, not
  ...
  - **A prose label is never generated.** Deriving `Client order ID` from
    `ClOrdID` means splitting on case and expanding `Cl` to `Client` and
    `Ord` to `order` — a guess about English, which is the same guess P4's
    Decided refuses for `identifier` to `id`. A label nobody can regenerate
    is a fact that rots the first time the dictionary is rebuilt, so the
    seed's hand-written labels go with the first regeneration.
...
- **P3-R9. `deprecated: true` and `removed: true`** mark the states the
...
- **P3-R10. Every key beyond `since` is optional.** An entry stating only
  `since` means "present, unchanged", which most versions are.
- **P3-R11. The field's own name and datatype MUST equal the newest entry's.**
  `set_lineage` refuses a disagreement, naming both sides, so the lineage is
  the authority and the field cannot drift from it.
- **P3-R12. `fix:aliases` is rewritten from the lineage's historical names**
  on that same call, so a query by an old spelling resolves through the
  index that already exists. Both derivations are the writer's, never a
  caller's: that is what makes them undriftable.
- **P3-R13. No `fix:since`, `fix:until` or `fix:deprecated` key.** `since()`
  is the first entry's, `until()` the `removed` one's, derived on read the
  way `FixId` is derived from branch and tag (N4).
- **P3-R14. The registry stays version-agnostic.** It holds every tag ever
...
- **P3-R15. `versions()` and `newest()` are derived,** not stored. The first
  ...
  what "FIX Latest" means for that dictionary (P3-R2).
...
4. Two entries at one version and different EPs order by EP (P3-R5);
   `newest()` answers the real pair, not a sentinel (P3-R2).
5. A query by `LastShares` and by `LastQty` answers the same field.
6. `field_at(&"4.2", "LastQty")` refuses.
7. A lineage disagreeing with the field's own name is refused (P3-R11).
...
- **P4-R10. A code's pedigree is Phase 3's pair,** stored as real numbers.
...
- **P4-R11. JSON, not separator-delimited text,** because metadata values
...
- **P5-R1. Per-key rules, because "merge" alone decides nothing.**
  ...
  | `fix:aliases` | union, ASCII-folded, incoming first — then **rewritten from the merged lineage** so P3-R12 holds |
  | `fix:description` | **never compared.** Incoming wins when it has one; stored kept when it does not |
  | `fix:lineage` | merged by pedigree: union, incoming wins an equal pair, re-sorted oldest-first, re-validated against the merged name and datatype (P3-R11) |
...
- **P6-R1. Every source URL is pinned to a commit, never `master`.** yggfin
  ...
  unreviewable.
- **P6-R2. The default output is `config/fix` at the repository root.** A
...
- **P6-R3. `--source`** takes a local directory or a URL base so a run is
  reproducible offline; the default is the pinned set.
- **P6-R4. Record the real version label, never "Latest".** The Orchestra
  ...
  and the dictionary does not (P3-R2). A regeneration against a later EP
  changes those numbers, which is the diff a reviewer wants.
- **P6-R5. Output is exactly the shard layout the store reads:** a
...
- **P6-R6. Scrape the whole registry, and store each kind where it can
  ...
  | groups | the `nested` tree, a List of an `item` Struct (P6-R14) | the counter carries the tag |
  | code sets | `fix:codes` on the field that declares them (P4) | a code set is a property of its field, not an entity |
  | datatypes | nothing is stored | they are `LOGICAL_NAMES`, which already *is* the FIX datatype table; the run **checks** every scraped datatype resolves and fails loudly on one that does not (P6-R11) |
...
- **P6-R7. Provenance in a manifest beside the trees,** one record per
...
- **P6-R8. A leaf beside the trees is safe:** the store descends only the
  two trees, so it is never listed. It must not carry the retired-layout
  name the store trips on.
- **P6-R9. Fill the branch manifest, not just the shards.** Every source
  that is a dialect states what P2-R12 bundles: a vendor orchestration its
  FIX version, a `.cfb` its version *and* its `SenderCompID`/`TargetCompID`.
  The standard branch declares nothing (P2-R13) — it is the specification,
  not a counterparty.
- **P6-R10. Pull one vendor dictionary into a non-standard branch in the same
...
- **P6-R14. Repeating groups become the nested tree as today:** a List of a
  non-null `item` Struct whose `fix:tag` is the counter's.
- **P6-R15. Priority ordering.** Merge sources lowest priority first so the
  highest wins by P5-R1.

### The standard header and trailer

- **P6-R16. Order, not nesting.** `FixMsg` lays a message flat (P7), so both
...
- **P6-R17. They are not registry entries.** A component has no tag and
...
- **P6-R18. The constants are the union across every scraped version,** in
...
2. Load the generated tree and write it back: byte-identical (P6-R5).
...
**Docs.** Provenance, version coverage, the FIXT omission (P3-R3), the
dropped component names (P6-R17) and the regeneration command, on the FIX
page.
...
- **P8-R4. The vocabulary is the union across every scraped version,**
  generated by Phase 6 the way the header constants are (P6-R16), so one
...
lowest priority first (P6-R15), so nothing in the core learns about sources.
