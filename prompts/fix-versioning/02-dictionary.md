...
**Goal.** Everything that fills a FIX dictionary and keeps it coherent: per-version
lineage on a field, its code set, the one merge that folds two definitions
together, and the generator that writes the committed dictionary from the
published sources.

**Depends.** Phase 1 (the `Version` value). Phases 3 → 4 → 5 → 6 run in that order.
...
- **P6-R5b. Fill the branch manifest, not just the shards.** Every source
  that is a dialect states what P2-R9b bundles: a vendor orchestration
  states its FIX version, and a `.cfb` states its version *and* its
  `SenderCompID` / `TargetCompID` on the root element. Write them into the
  branch manifest in the same run. The standard branch declares nothing
  (P2-R9c) - it is the specification, not a counterparty.
...
3b. The vendor branch's manifest entry carries its declared version, and the
