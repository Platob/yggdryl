...
| `xml/` | the XML structured codec over `Scalar` - a document is the record naming its root element, `@name` an attribute, `#text` an element's own text beside attributes or children, a repeated element a sequence, a self-closed element null and an emptied one the empty text, every leaf text - `mod.rs` the doors, `parser.rs` the quick-xml event fold, `wire.rs` the writer and the field-directed reshaping (a repeated element read once is one item, absent is the empty sequence, text trimmed and empty text null under a non-text leaf); `scanner.rs` beside them, private and under the `aws` feature, the deterministic scanner for the small fixed-shape XML documents S3, Azure Blob Storage and STS answer, knowing the name of no element, each reader naming its own vocabulary over it; `element.rs` the namespace-aware `Element`/`Scope` view over a parsed document, and `soap/` the SOAP 1.1 envelope - `Envelope`, `Fault`, `Fragment`, the streaming `EnvelopeWriter` - with `soap/http.rs` the one-request HTTP/1.1 reading and chunked writing its binding needs: protocol vocabulary no medium owns, so it sits beside the codec rather than under `xmla/` |
...
- Encoding comes from `MediaType` through `RecordOptions`, with no format
  ...
  paths.
- Plain-text rows are the sixteen event columns `EventColumn::ALL` names -
...
- **A code's identity is its extension name, not its storage.** That split
  ...
  The retired `yggdryl.currency` name is refused before that foreign-extension fallback.
...
- Root `mkdocs.yml` is authoritative - strict build, nav, and links change
  ...
  plain text, JSON, YAML, TOML, XML, XML for Analysis, Iceberg - then Compression (gzip, zlib, zstd)
