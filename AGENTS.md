...
| `soap/` | the SOAP 1.1 envelope over the XML codec - `Envelope`, `Fault`, `Fragment`, the streaming `EnvelopeWriter` - and, in `http.rs`, the one-request HTTP/1.1 reading and chunked writing its binding needs; protocol vocabulary no medium owns, a root folder of its own name like every implementation, which `xmla/` speaks over |
| `xmla/` | the XML for Analysis 1.1 medium and provider: `vocabulary.rs` the methods, request types, properties, restrictions and enumerations, `request.rs` `Discover`, `Execute`, `Request` and the session headers, `response.rs` `Response`, `XmlaError` and the fault door, `rowset.rs` the rowset document - `Rowset`, `XsdType`, the `_xHHHH_` name escape - written cell by cell off the column leaves and read back under the schema it carries or a declared field, `dbtype.rs` the OLE DB type codes `DBSCHEMA_COLUMNS` states, `definitions.rs` the schema rowsets the provider answers, `options.rs` `XmlaOptions`, `media.rs` `Xmla<H>` the `.xmla` handle, `catalog.rs` `Catalog`/`Table` over a folder of record media (a leaf with record options is a table, a folder at the root a schema unless it is laid out as a table format, a folder under a schema a table; a ZIP archive's members are named by their own names), `service.rs` the provider answering a request's bytes - Discover over the definitions, Execute through the expression `Plan` against the table a statement names - and `server.rs` the SOAP 1.1 HTTP binding on a socket; `ygg xmla serve` in `cli/` is its terminal |
