...
| `graph/` | the graph vocabulary: `element.rs` holds `Element` - an element's `Uuid` and its parents' UUIDs, read and written - and `TimeElement`, an element with an instant (`unix`, `i128` nanoseconds since the epoch, UTC) and a `hashcode`; signatures only, no storage and no walk |
