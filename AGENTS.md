...
- `Serie` <-> Arrow array, batch or reader, through `from_arrow_array`/
  ...
  `from_arrow_reader`/`into_arrow_reader`, by sharing buffers: no row is read
  in either direction, the door proving layout and absence on the buffers and
  a value only where the layout is not the datatype's whole contract.
