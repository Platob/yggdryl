...
| many values | `Serie` | a schema-free `Run` - what a row is - or the Arrow buffers of one `Field`, nested as `Serie` children; `Scalar::Sequence` holds one, every column leaf is a `SerieValue`, and identity is the rows alone |
