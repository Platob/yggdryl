...
| `market/` | the market's products: what the market did, read out of what a venue said - `order.rs`, `quote.rs`, `execution.rs`, `trade.rs` and `book.rs`, each a trait (`Order`, `Quote`, `Execution`, `Trade`, `Book`: the product's own accessors and mutators, and the readings the stated facts imply - an order's pricing and what is left, a quote's lanes, a book's mid, spread, microprice and imbalance) beside its holder (`OrderData`, `QuoteData`, `ExecutionData`, `TradeData`, `BookData`), one value each with its own layout, its own Arrow row and its own identity, every one a graph event exactly as a message is, so its row opens with the sixteen `EventColumn`s and joins the message table on `srcuuids` to `curruuid`; `symbol.rs` the `Symbol` an instrument is keyed by, `GLOBAL` for none; `statement.rs` the `Statement` one value over the four products that state something to a ladder; `iterator.rs` the `BookIterator`, one book per symbol per instant read out of a stream of statements, the live orders and quotes kept per symbol, the prints counted, the grid optional; `product.rs` what the five share - `MarketColumn`, the columns the market's facts are stated in, `Product`, what a product answers about its row, `Folded` and `Walked`, the fold of two statements of one product and the one walk over a fallible stream, and the row doors every Arrow twin composes. Nothing about a protocol: the FIX reading of a product - one door per product over a stream of messages, one stream of every statement, one over an Arrow reader of message rows, and the message a product states where one does - is `fix/market.rs`, so the folder stays a vocabulary a second protocol could feed |
