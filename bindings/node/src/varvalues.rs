    ...
    ($Scalar:ident, $Serie:ident, $E:ty, $js:ty, $to:path, $from:path, $lit:literal,
     scalar_extra = { $($scalar_extra:tt)* }) => {
        ...
        impl $Serie {
            ...
            // ---- Phase 8: reshape + row-selection (no arithmetic on a var column) ------------
            ...
            /// no-op clone). The value is a `string` (utf8) / `Buffer` (binary), matching the column.
            #[napi]
            ...
            /// var column; reconstruct with the resulting class's `deserializeBytes`).
            #[napi]
            ...
            /// cells. Throws on an out-of-range offset or an incompatible source cell. `other` must be a
            /// `Serie` wrapper.
            #[napi]
