...
/// `yggdryl_scalar::$ty`. The width-dependent constructor, `values` and `valueAt`
...
/// Generates the width-dependent constructor, `values` and `valueAt` of an 8-32 bit
...
macro_rules! int_serie_wire_number_scalar {
    ($ty:ident, $native:ty, $name:literal) => {
        ...
        impl $ty {
            ...
            #[napi]
            pub fn value_at(&self, index: i64) -> Result<i64> {
                self.inner
                    .value_at::<$native>(index_to_usize(index)?)
                    .map(i64::from)
                    .map_err(data_error)
            }
...
/// Generates the wire-dependent constructor, `toArray` and `valueAt` of a float serie
...
macro_rules! float_serie_wire_number_scalar {
    ($ty:ident, $native:ty, $name:literal) => {
        ...
        impl $ty {
            ...
            /// the index is negative, or the index is past the end.
            #[napi]
            pub fn value_at(&self, index: i64) -> Result<f64> {
                self.inner
                    .value_at::<$native>(index_to_usize(index)?)
                    .map(<$native>::to_wire)
                    .map_err(data_error)
...
impl Int64Serie {
    ...
    /// the index is negative, or the index is past the end.
    #[napi]
    pub fn value_at(&self, index: i64) -> Result<BigInt> {
        self.inner
            .value_at::<i64>(index_to_usize(index)?)
            .map(BigInt::from)
            .map_err(data_error)
    }
...
impl UInt64Serie {
    ...
    pub fn value_at(&self, index: i64) -> Result<BigInt> {
        self.inner
            .value_at::<u64>(index_to_usize(index)?)
            .map(BigInt::from)
            .map_err(data_error)
    }
