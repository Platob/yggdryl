...
impl Inferred {
    ...
    fn arrow_dtype(&self) -> arrow_schema::DataType {
        match self {
            ...
            Inferred::Utf8(_) => yggdryl_dtype::Utf8Type.to_arrow(),
            Inferred::Serie(_) => {
...
fn infer(value: &Bound<'_, PyAny>) -> PyResult<Inferred> {
    ...
    if value.is_instance_of::<PyString>() {
        return Ok(Inferred::Utf8(value.extract()?));
    }
...
fn dtype(py: Python<'_>, value: &Bound<'_, PyAny>) -> PyResult<PyObject> {
    ...
    Ok(match infer(value)? {
        ...
        Inferred::Utf8(_) => Py::new(py, crate::dtype::Utf8Type::default())?.into_any(),
