...
///
/// The backing is an `arrow_buffer::ScalarBuffer<$ty>` (the core is Arrow-backed), so
/// an Arrow buffer is wrapped **zero-copy** and an owned `Vec` becomes one without a
/// copy; it hands out an aligned `&[$ty]` view.
macro_rules! primitive_buffer {
    ($name:ident, $ty:ty) => {
        ...
        ///
        #[doc = concat!("```")]
        #[doc = concat!("use yggdryl_core::", stringify!($name), ";")]
