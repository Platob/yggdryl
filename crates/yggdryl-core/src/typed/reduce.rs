...
//! (`Int32`, `Float64`, …) maps its `sum`/`min`/`max`/`mean` to the matching width-specific
//! `Aggregate` method on the underlying source, so a [`Serie`](super::Serie) reduces over its data
//! buffer with the same allocation-free, LLVM-vectorized, NaN-safe kernels every `IOBase` runs — no
//! per-column loop of its own. The `bool` [`Bit`](super::fixedbit::Bit) type is **not** `Reduce`
//! (booleans do not sum).
