...
use yggdryl_core::{
    ByteBuffer, F64Buffer, I16Buffer, I32Buffer, I64Buffer, IOBase, IOCursor, IoError, TypedCursor,
    TypedIOBase, U8Buffer, Whence,
};

#[test]
fn typed_cursor_round_trips_per_type() {
    let mut i16c = I16Buffer::from_slice(&[-1, 2, -3]).cursor();
    assert_eq!(i16c.pread_array(3, Whence::Start).unwrap(), vec![-1, 2, -3]);

    let mut i64c = I64Buffer::from_slice(&[i64::MIN, 0, i64::MAX]).cursor();
    assert_eq!(
        i64c.pread_array(3, Whence::Start).unwrap(),
        vec![i64::MIN, 0, i64::MAX]
    );

    let mut f64c = F64Buffer::from_slice(&[1.5, -2.5, f64::INFINITY]).cursor();
    assert_eq!(
    ...
    let mut u8c = U8Buffer::from_slice(&[7, 8, 9]).cursor();
...
fn negative_typed_seek_before_start_is_rejected() {
    let mut cursor = I32Buffer::from_slice(&[1, 2]).cursor();
...
fn write_past_end_fills_the_gap_with_the_type_default() {
    let mut cursor = I32Buffer::new().cursor();
...
fn default_accessors_describe_the_fill() {
    let cursor = I32Buffer::new().cursor();
...
fn typed_arrays_truncate_on_over_request() {
    let mut cursor = I16Buffer::from_slice(&[1, 2, 3]).cursor();
...
fn write_is_copy_on_write_leaving_the_buffer_intact() {
    ...
    assert_eq!(buffer.as_slice(), &[1, 2, 3], "source buffer untouched");
}
