...
fn read_first_all_pairs_contains() {
    ...
    assert!(uri.param_all("missing").is_empty());

    assert_eq!(uri.params(), vec![("a", "1"), ("b", "2"), ("a", "3")]);
    assert!(uri.has_param("a"));
    assert!(!uri.has_param("missing"));
}

#[test]
fn no_query_reads_empty() {
    let uri = Uri::parse_str("http://h/p").unwrap();
    assert_eq!(uri.param("a"), None);
    assert!(uri.param_all("a").is_empty());
...
fn remove_drops_all_occurrences_and_clears_empty_query() {
    let mut uri = Uri::parse_str("http://h/p?a=1&b=2&a=3").unwrap();
    assert!(uri.remove_param("a")); // returns true when something was removed
    assert_eq!(uri.query(), Some("b=2"));

    assert!(uri.remove_param("b"));
    assert_eq!(uri.query(), None); // last param gone -> query cleared entirely
    assert_eq!(uri.to_string(), "http://h/p");

    assert!(!uri.remove_param("anything")); // no-op on an absent key
}
...
fn value_may_embed_equals() {
    ...
    assert_eq!(uri.param("token"), Some("a=b=c"));
}

#[test]
fn params_build_a_map() {
    let uri = Uri::parse_str("http://h/p?a=1&b=2&c=3").unwrap();
    let map: HashMap<&str, &str> = uri.params().into_iter().collect();
...
#[test]
fn url_mirrors_the_param_surface() {
    let mut url = Url::parse_str("https://h/p?a=1&a=2").unwrap();
    assert_eq!(url.param("a"), Some("1"));
    assert_eq!(url.param_all("a"), vec!["1", "2"]);
    assert!(url.has_param("a"));

    url.set_param("a", "9");
    assert_eq!(url.param("a"), Some("9"));
    assert!(url.remove_param("a"));
    assert_eq!(url.params(), Vec::<(&str, &str)>::new());
    assert_eq!(url.with_param("k", "v").to_string(), "https://h/p?k=v");
}

#[test]
fn set_params_bulk_update() {
    let mut uri = Uri::parse_str("http://h/p?a=1&b=2&a=3").unwrap();
    uri.set_params(&[("a", "9"), ("c", "7"), ("d", "0")]);
...
#[test]
fn normalize_params_sorts_and_cleans() {
...
fn grouped_rebuilds_a_multimap() {
    ...
    let map: HashMap<&str, Vec<&str>> = uri.params_grouped().into_iter().collect();
