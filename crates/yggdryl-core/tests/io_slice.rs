...
fn window_rebases_positions_and_reports_its_own_len() {
    ...
    assert_eq!(io.pread(0, Whence::Start).unwrap(), 20);
    assert_eq!(io.pread(2, Whence::Start).unwrap(), 40);
    // End is measured from the window's end.
    assert_eq!(io.pread(1, Whence::End).unwrap(), 40);
}
...
fn writes_stay_within_the_window() {
    ...
    io.pwrite(0, Whence::Start, 21).unwrap();
    ...
    assert_eq!(io.pwrite(3, Whence::Start, 9), Err(IoError::OutOfBounds));
