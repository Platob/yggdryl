...
#[test]
fn param_value_is_encoded_on_store_and_decoded_on_read() {
    let mut uri = Uri::parse_str("http://h/p").unwrap();
    uri.set_param("q", "a b&c=d#e");
    ...
    assert_eq!(uri.param("q"), Some("a%20b%26c%3Dd%23e"));
    assert_eq!(uri.param_decoded("q").as_deref(), Some("a b&c=d#e"));
}

#[test]
fn set_get_round_trips_through_decode() {
    for value in ["plain", "a b", "100%", "x&y=z", "café", "π=3.14", ""] {
        let uri = Uri::parse_str("http://h/p").unwrap().with_param("k", value);
        assert_eq!(
            uri.param_decoded("k").as_deref(),
            Some(value),
...
fn key_with_special_chars_is_encoded_and_looked_up_decoded() {
    let uri = Uri::parse_str("http://h/p").unwrap().with_param("a b", "1");
    assert_eq!(uri.query(), Some("a%20b=1"));
    assert_eq!(uri.param("a b"), Some("1")); // the lookup key is encoded internally
    assert!(uri.has_param("a b"));
    assert!(!uri.has_param("a%20b")); // the raw encoded string is a different key
}
...
fn percent_literal_round_trips() {
    ...
    assert_eq!(uri.param_decoded("p").as_deref(), Some("100%"));
}
...
fn parse_keeps_input_verbatim() {
    ...
    assert_eq!(uri.param("q"), Some("a%20b")); // as given
    assert_eq!(uri.param_decoded("q").as_deref(), Some("a b"));
...
fn bulk_update_encodes_each_value() {
    let uri = Uri::parse_str("http://h/p")
        .unwrap()
        .with_params(&[("a", "1 2"), ("b", "x&y")]);
    assert_eq!(uri.query(), Some("a=1%202&b=x%26y"));
    assert_eq!(uri.param_decoded("a").as_deref(), Some("1 2"));
    assert_eq!(uri.param_decoded("b").as_deref(), Some("x&y"));
}
...
fn decoded_map_view() {
    let uri = Uri::parse_str("http://h/p")
        ...
        .with_param("name", "a b")
        .with_param("tag", "x&y");
    let decoded = uri.params_decoded();
...
fn malformed_escape_is_left_verbatim_on_decode() {
    ...
    assert_eq!(uri.param_decoded("q").as_deref(), Some("50%ZZ ok"));
}
