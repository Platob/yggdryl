...
#[test]
fn port_boundaries() {
    assert_eq!(Uri::parse("//h:0").unwrap().port(), Some(0));
    assert_eq!(Uri::parse("//h:1").unwrap().port(), Some(1));
    assert_eq!(Uri::parse("//h:65535").unwrap().port(), Some(65535));
    // 65536 is one past `u16::MAX` — a guided error naming the offending value.
    let err = Uri::parse("//h:65536").unwrap_err();
...
fn multiple_question_and_hash_split_on_the_first() {
    ...
    let uri = Uri::parse("p?a?b#c#d").unwrap();
...
fn percent_encoding_is_kept_verbatim() {
    ...
    let uri = Uri::parse("http://h/a%20b?x=%3D#f%23").unwrap();
...
fn empty_query_and_fragment_are_present_but_empty() {
    let q = Uri::parse("http://h/p?").unwrap();
    assert_eq!(q.query(), Some(""));
    assert_eq!(q.fragment(), None);

    let f = Uri::parse("http://h/p#").unwrap();
...
fn distinct_uris_are_distinct_map_keys() {
    ...
    for (i, s) in entries.iter().enumerate() {
        assert_eq!(map.get(&Uri::parse(s).unwrap()), Some(&i));
    }
}
