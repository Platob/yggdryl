...
///
/// Like its [`StructType`], it is dynamic — the children are only known at
/// runtime — so its [`Value`](RawScalar::Value) is the borrowed slice of column
/// [`ArrayRef`]s (each of length one), and construction validates the columns
...
impl StructScalar {
    ...
    pub fn new(data_type: StructType, columns: Vec<ArrayRef>) -> Result<Self, DataError> {
        let fields = super::RawStruct::fields(&data_type);
...
impl RawScalar<StructType> for StructScalar {
    ...
    fn to_arrow(&self) -> ArrayRef {
        let fields = super::RawStruct::fields(&self.data_type);
    ...
    fn from_arrow(array: &dyn arrow_array::Array) -> Result<Self, DataError> {
        ...
        let data_type = crate::RawDataType::from_arrow(arrow_array::Array::data_type(array))?;
        let array = array
            .as_any()
