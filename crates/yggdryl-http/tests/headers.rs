...
fn byte_and_string_access() {
    ...
    assert_eq!(headers.insert(b"unit".to_vec(), b"us".to_vec()), Some(b"ms".to_vec()));
...
fn common_key_accessors() {
    ...
    assert_eq!(headers.content_type(), Some(b"application/x.int64".as_slice()));
...
fn byte_round_trip_and_ordering_is_deterministic() {
    ...
    let a = Headers::from_pairs([(b"b".to_vec(), b"2".to_vec()), (b"a".to_vec(), b"1".to_vec())]);
    let b = Headers::from_pairs([(b"a".to_vec(), b"1".to_vec()), (b"b".to_vec(), b"2".to_vec())]);
    assert_eq!(a, b);
    ...
    assert_eq!(Headers::deserialize_bytes(&empty.serialize_bytes()).unwrap(), empty);

    // Non-UTF-8 keys/values survive.
    let binary = Headers::from_pairs([(vec![0xFF, 0x00], vec![0x00, 0xFE])]);
    assert_eq!(Headers::deserialize_bytes(&binary.serialize_bytes()).unwrap(), binary);
}

#[test]
fn truncated_deserialize_is_guided() {
    // A count promising one entry but no bytes for the key length.
    assert_eq!(Headers::deserialize_bytes(&[1, 0, 0, 0]).unwrap_err(), HeadersError::Truncated);
    ...
    assert_eq!(Headers::deserialize_bytes(&[]).unwrap_err(), HeadersError::Truncated);
}
...
fn headers_based_full_surface() {
    ...
    column.get_header_mut(b"unit").unwrap().extend_from_slice(b"ec");
    ...
    assert_eq!(column.remove_header(Headers::CONTENT_TYPE), Some(b"text/plain".to_vec()));
