//! The [`Field`] base trait: a named, nullable column of a
...
/// use yggdryl_field::{arrow_schema, Field};
///
/// #[derive(Debug)]
/// struct Column {
///     name: String,
...
pub trait Field<D: DataType>: std::fmt::Debug + Send + Sync {
    ...
    /// inverse of [`to_arrow`](Field::to_arrow) on the metadata-free fields that
    /// method produces, and the Arrow factory. A field of a different Arrow data
    ...
    /// (see [`to_arrow`](Field::to_arrow)) and is deliberately dropped — logged as a
    /// `warn` when the `log` cargo feature is on.
    fn from_arrow(field: &arrow_schema::Field) -> Result<Self, DataError>
    where
        Self: Sized;
}
