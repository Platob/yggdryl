...
impl FixedSizeBinaryType {
    /// A fixed-size binary type whose values are `byte_size` bytes each.
    pub fn new(byte_size: i32) -> Self {
        Self { byte_size }
    }

    /// The fixed element width in bytes.
    pub fn byte_size(&self) -> i32 {
        self.byte_size
    }

    /// The fixed element width in bytes, widened to 64 bits.
    pub fn large_byte_size(&self) -> i64 {
        i64::from(self.byte_size)
    }

    /// Returns a copy with a new element width.
    pub fn with_byte_size(&self, byte_size: i32) -> Self {
        Self { byte_size }
    }

    /// Returns a copy with a new element width given as a 64-bit value, clamping to
    ...
    pub fn with_large_byte_size(&self, byte_size: i64) -> Self {
        let byte_size = i32::try_from(byte_size).unwrap_or_else(|_| {
            crate::log_event!(
                warn,
                "FixedSizeBinaryType::with_large_byte_size {byte_size} exceeds i32; clamping to i32::MAX"
            );
            i32::MAX
        });
        Self { byte_size }
