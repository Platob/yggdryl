//! The timestamp data type, one per time unit.
...
/// An instant as a 64-bit offset since the UNIX epoch at the resolution `U`,
/// with an optional timezone — one timestamp type per [`TimeUnit`]
/// (`Timestamp<Nanosecond>` through `Timestamp<Year>`), anchored on
/// [`Int64`].
///
...
///
/// ```
/// use yggdryl_schema::{DataType, Millisecond, Minute, Timestamp};
///
/// let millis = Timestamp::from_parts(Millisecond, Some("UTC".into()));
/// assert_eq!(Timestamp::from_arrow(&millis.to_arrow()), Ok(millis));
///
/// let minutes = Timestamp::from_parts(Minute, None);
...
pub struct Timestamp<U: TimeUnit> {
    unit: U,
...
impl<U: TimeUnit> Timestamp<U> {
    ...
    pub fn from_parts(unit: U, timezone: Option<Arc<str>>) -> Self {
        Self { unit, timezone }
    }
    ...
    /// from `self`.
    pub fn copy(&self, unit: Option<U>, timezone: Option<Option<Arc<str>>>) -> Self {
        Self::from_parts(
...
impl<U: TimeUnit> DataType for Timestamp<U> {
...
impl<U: TimeUnit> PrimitiveType for Timestamp<U> {
    type Native = i64;
    const BIT_WIDTH: usize = 64;
}

impl<U: TimeUnit> LogicalType for Timestamp<U> {
    type Physical = Int64;
