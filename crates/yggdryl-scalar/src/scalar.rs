...
/// [`DataType<T>`](yggdryl_dtype::DataType), whose owned-value byte codec would force
/// `T: Sized` — so a string scalar can expose the borrowed `Option<&str>` while its
/// data type still codecs owned `String`s through
/// [`DataType<String>`](yggdryl_dtype::DataType).
///
/// ```
/// use yggdryl_scalar::{Int64, RawScalar, Scalar};
///
