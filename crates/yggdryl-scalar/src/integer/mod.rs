...
//! each is generated by the focused crate-internal [`int_scalar!`] macro —
//! keeping every per-type file a single invocation while the logic lives in one
//! place.

/// Generates a single, possibly-null scalar `$ty` over the `yggdryl-dtype` data
/// type of the same name (native `$native`), with `$name` used in its
/// documentation and `$array` the matching [`arrow_array`] primitive array
/// (`to_arrow` / `from_arrow`). The scalar also converts from its native value —
/// `From<$native>` and `From<Option<$native>>` (where `None` is the null scalar)
/// — and the data type gains its [`DefaultScalar`](crate::DefaultScalar) impl.
macro_rules! int_scalar {
    ($ty:ident, $native:ty, $name:literal, $array:ident) => {
        ...
        impl $crate::RawScalar<::yggdryl_dtype::$ty> for $ty {
            type Value = $native;
        ...
        impl $crate::Scalar<$native> for $ty {
            type Type = ::yggdryl_dtype::$ty;
        }

        impl $crate::DefaultScalar<$native> for ::yggdryl_dtype::$ty {
            type Scalar = $ty;
