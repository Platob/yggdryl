...
//! [`TypedSerieType<UInt16Type>`](yggdryl_dtype::TypedSerieType) data type, holding its
...
//! assert_eq!(numbers.scalar_at(1), Some(UInt16Scalar::new(2)));
//! assert_eq!(numbers.data_type().name(), "list");
//!
//! // Nulls are per element, read null-aware.
//! let sparse = UInt16Serie::from(vec![Some(1), None]);
//! assert!(sparse.value_at::<u16>(1).is_err()); // a null element holds no value
//! assert_eq!(sparse.scalar_at(1), Some(UInt16Scalar::null()));
//!
