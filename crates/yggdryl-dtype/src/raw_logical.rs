...
/// is preserved for zero-cost access, mirroring `yggdryl-field`'s `RawField` and
/// `yggdryl-scalar`'s `RawScalar`; a logical type whose values also have a native
/// representation implements the typed [`Logical`](crate::Logical).
///
/// ```
/// use yggdryl_dtype::{arrow_schema, DataError, Int64, RawDataType, RawLogical};
///
