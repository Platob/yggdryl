...
//! each reuses the integer family's crate-internal `int_data_type!` macro (`f32` /
//! `f64` carry the same `to_le_bytes` / `from_le_bytes` surface). The matching
//! fields and scalars live in `yggdryl-field` and `yggdryl-scalar`, under the
//! `Float64Field` / `Float64Scalar` naming convention.
