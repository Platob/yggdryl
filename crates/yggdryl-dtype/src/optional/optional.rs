...
/// type — and names the value data type as the associated
/// [`ValueType`](Optional::ValueType) so the concrete type is preserved for
/// zero-cost access, mirroring `yggdryl-field`'s `Field` and `yggdryl-scalar`'s
/// `Scalar`. A value type with a codec also gets the typed
/// [`TypedOptional`](crate::TypedOptional) layer.
///
/// ```
